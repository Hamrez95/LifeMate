...
projectPendingTreatmentCreateReminders({
  ...
  for (final raw in pendingCreates) {
    ...
    final first = _maxDate(startDate, DateTime(localNow.year, localNow.month, localNow.day));
    final lastBound = endDate ?? DateTime(localHorizon.year, localHorizon.month, localHorizon.day);
    final last = _minDate(lastBound, DateTime(localHorizon.year, localHorizon.month, localHorizon.day));
    if (last.isBefore(first)) continue;

    for (var date = first; !date.isAfter(last); date = date.add(const Duration(days: 1))) {
      for (final schedule in schedules) {
        ...
        final triggerUtc = scheduledLocal.toUtc().subtract(Duration(minutes: lead));
        if (!triggerUtc.isAfter(normalizedNow) || triggerUtc.isAfter(horizonEnd)) continue;

        final dateText = _dateText(date);
        final key = 'pending-treatment-create-reminder:$requestId:$dateText:${schedule.localTime}';
        if (!seen.add(key)) continue;
        final revision = LifeMateReminderIdentity.stableRevisionFor(
          'pending-treatment-create-v1|$requestId|$dateText|${schedule.localTime}|$zoneName|lead:$lead',
        );
        result.add(
...
DateTime _maxDate(DateTime left, DateTime right) => left.isAfter(right) ? left : right;
DateTime _minDate(DateTime left, DateTime right) => left.isBefore(right) ? left : right;
