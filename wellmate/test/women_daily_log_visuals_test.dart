...
void main() {
  test('daily log serializes canonical language-neutral values', () {
    ...
    expect(
      draft.toApiBody()['symptoms'],
      containsAll(<String>['cramps', 'bloating']),
    );
  });
  ...
  testWidgets('no symptom is exclusive with other symptoms', (tester) async {
      ...
      MaterialApp(
        home: Builder(
          builder: (context) => Scaffold(
            body: TextButton(
              onPressed: () async {
                result = await showWomenDailyLogSheet(
                  context,
                  loggedOn: DateTime(2026, 8, 30),
                );
              },
    ...
    await tester.ensureVisible(find.text('No symptom'));
    await tester.pumpAndSettle();
