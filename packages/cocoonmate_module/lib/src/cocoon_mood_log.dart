...
class _CocoonMoodLogScreenState extends State<CocoonMoodLogScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _state = error.statusCode == 0
        ...
        body: SafeArea(
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Text(t('How are you feeling?', 'از نظر روحی چه حسی داری؟'),
                    style: Theme.of(context).textTheme.headlineSmall),
                const SizedBox(height: 8),
                Text(
                  t('This is a personal wellbeing check-in, not a diagnosis.',
                ...
                FilledButton.icon(
                  ...
                  icon: _busy ? const SizedBox.square(dimension: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.check_rounded),
...
class _MoodOption extends StatelessWidget {
  const _MoodOption({required this.fa, required this.mood, required this.selected, required this.enabled, required this.onTap});
  final bool fa;
    ...
    return Semantics(
      ...
      child: OutlinedButton(
        ...
        style: OutlinedButton.styleFrom(
          ...
          side: BorderSide(color: selected ? CocoonTheme.skyStrong : CocoonTheme.line),
        ),
        child: Text(label),
      ),
    );
...
class _MoodStatus extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final text = switch (state) {
      CocoonMoodSubmitState.queued => fa ? 'ثبت شد و در انتظار همگام‌سازی است.' : 'Saved and waiting to sync.',
      CocoonMoodSubmitState.confirmed => fa ? 'ثبت و تأیید شد.' : 'Saved and server-confirmed.',
      CocoonMoodSubmitState.error => fa ? 'ثبت انجام نشد؛ دوباره تلاش کن.' : 'Could not save. Try again.',
      CocoonMoodSubmitState.offline => fa ? 'اتصال برقرار نیست؛ دوباره تلاش کن.' : 'You are offline. Try again when connected.',
      _ => '',
    };
