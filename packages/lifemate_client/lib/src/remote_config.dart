...
typedef LifeMateRemoteConfigSubjectProvider = String? Function();
typedef LifeMateRemoteConfigCacheRead = Future<String?> Function(String key);
typedef LifeMateRemoteConfigCacheWrite = Future<void> Function(
  String key,
  String value,
);
...
class LifeMateRemoteControl {
  ...
  factory LifeMateRemoteControl.fromJson(Map<String, dynamic> json) =>
      LifeMateRemoteControl(
        ...
        value: json['value'],
        definitionVersion:
            int.tryParse(json['definitionVersion']?.toString() ?? '') ?? 0,
...
class LifeMateUpdatePolicy {
  ...
  factory LifeMateUpdatePolicy.fromJson(Map<String, dynamic> json) {
    ...
    return LifeMateUpdatePolicy(
      ...
      policyVersion:
          int.tryParse(json['policyVersion']?.toString() ?? '') ?? 0,
    );
  }
...
class LifeMateRuntimeConfigSnapshot {
  ...
  factory LifeMateRuntimeConfigSnapshot.fromJson(
    ...
    final rawTtl = int.tryParse(json['cacheTtlSeconds']?.toString() ?? '') ?? 60;
        ...
        for (final raw in (json['controls'] as List<dynamic>? ?? const []))
          if (raw is Map)
            Map<String, dynamic>.from(raw)['key']?.toString() ?? '':
                LifeMateRemoteControl.fromJson(
                  Map<String, dynamic>.from(raw),
                ),
      }..remove(''),
      updatePolicy: LifeMateUpdatePolicy.fromJson(
        Map<String, dynamic>.from(
          json['updatePolicy'] as Map? ?? const {},
        ),
      ),
  ...
  Duration ageAt(DateTime now) => now.toUtc().difference(fetchedAtUtc);

  bool isFresh(DateTime now) {
    final age = ageAt(now);
    if (age < const Duration(minutes: -5)) return false;
    return age <= Duration(seconds: cacheTtlSeconds);
  }

  bool isTrustedForUpdatePolicy(
    DateTime now, {
    Duration maximumAge = const Duration(hours: 24),
  }) {
    final age = ageAt(now);
    return age >= const Duration(minutes: -5) && age <= maximumAge;
  }

  bool boolFlag(String key, {bool defaultValue = false, DateTime? now}) {
    final control = controls[key];
    if (control == null ||
        control.kind != 'FeatureFlag' ||
        control.valueType != 'Boolean') {
      return defaultValue;
    }
    final staleFor = (now ?? DateTime.now()).toUtc().difference(fetchedAtUtc);
    if (staleFor > const Duration(hours: 24) && control.failClosed) {
      return false;
    }
    return control.value is bool
        ? control.value as bool
        : (control.failClosed ? false : defaultValue);
  }
...
class LifeMateRemoteConfigClient {
  LifeMateRemoteConfigClient({
    ...
    required LifeMateRemoteConfigSubjectProvider cacheSubject,
    this.beta = false,
    String? platform,
  ...
  factory LifeMateRemoteConfigClient.fromEnvironment({
    ...
    return LifeMateRemoteConfigClient(
      ...
      cacheSubject: () => Supabase.instance.client.auth.currentUser?.id,
    );
  }

  static const _storage = FlutterSecureStorage();
  static const _timeout = Duration(seconds: 12);
  static final _subjectPattern = RegExp(
    r'^[0-9a-fA-F-]{16,64}$',
  );
  ...
  final LifeMateRemoteConfigSubjectProvider _cacheSubject;
  final http.Client _http;
  final LifeMateRemoteConfigCacheRead _cacheRead;
  final LifeMateRemoteConfigCacheWrite _cacheWrite;

  String? get _cacheKey {
    final subject = _cacheSubject()?.trim().toLowerCase();
    if (subject == null || !_subjectPattern.hasMatch(subject)) return null;
    return 'lifemate.runtime-config.$subject.$product.$platform';
  }
  ...
  Future<LifeMateRuntimeConfigSnapshot> load({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (cacheKey != null) {
        await _cacheWrite(cacheKey, jsonEncode(fresh.toJson()));
      }
      return fresh;
    } catch (_) {
  ...
  Future<void> recordVersionPresence({
    String? buildNumber,
    ...
    final uri = _baseUri.replace(
      path:
          '${_baseUri.path.replaceFirst(RegExp(r'/$'), '')}/api/v1/product/version-presence',
    );
  ...
  Future<LifeMateRuntimeConfigSnapshot> _fetch() async {
    final token = _requireToken();
    final path =
        '${_baseUri.path.replaceFirst(RegExp(r'/$'), '')}/api/v1/product/runtime-config';
    final uri = _baseUri.replace(
      path: path,
      queryParameters: {
        'product': product,
  ...
  Future<LifeMateRuntimeConfigSnapshot?> _readCached(String key) async {
    try {
      final raw = await _cacheRead(key);
      if (raw == null || raw.isEmpty) return null;
      final decoded = jsonDecode(raw);
      if (decoded is! Map) return null;
      final snapshot = LifeMateRuntimeConfigSnapshot.fromJson(
        Map<String, dynamic>.from(decoded),
        fromCache: true,
      );
      if (snapshot.product != product || snapshot.platform != platform) {
        return null;
      }
      return snapshot;
    } catch (_) {
...
String _buildFromVersion(String version) {
  ...
  return plus < 0 || plus == version.length - 1
      ? 'unknown'
