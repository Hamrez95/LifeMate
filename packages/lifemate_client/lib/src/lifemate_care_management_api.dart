...
class LifeMateCareManagementApi {
  ...
  Future<Map<String, dynamic>> getRelationshipPermission({required String relationshipId}) async =>
      _object(await _request('GET', '/api/v1/relationships/$relationshipId/health-record-permission'));

  Future<Map<String, dynamic>> updateHealthRecordPermission({
    required String relationshipId,
    required bool enabled,
    bool confirmConsent = false,
  }) async => _object(await _request(
    'PATCH',
  ...
  Future<List<Map<String, dynamic>>> getTreatmentPlans({required String patientUserId}) async =>
      _list(await _request('GET', '/api/v1/patients/$patientUserId/treatment-plans'));
  ...
  Future<Map<String, dynamic>> createTreatmentPlan({
    ...
    int patientReminderMinutesBefore = LifeMateReminderLeadTimes.defaultPatientMinutes,
    int caregiverReminderMinutesBefore = LifeMateReminderLeadTimes.defaultCaregiverMinutes,
  }) async => _object(await _request(
    'POST',
    ...
    body: {
      ...
      'recurrenceStartLocalTime': recurrenceStartLocalTime?.trim(),
  ...
  Future<void> deleteTreatmentPlan({required String patientUserId, required String treatmentPlanId, required int version}) async {
    await _request('DELETE', '/api/v1/patients/$patientUserId/treatment-plans/$treatmentPlanId', body: {'version': version});
  }

  Future<List<Map<String, dynamic>>> getCareEvents({required String patientUserId}) async =>
      _list(await _request('GET', '/api/v1/patients/$patientUserId/care-events'));
  ...
  Future<void> deleteCareEvent({required String patientUserId, required String eventId, required int version}) async {
    await _request('DELETE', '/api/v1/patients/$patientUserId/care-events/$eventId', body: {'version': version});
  }
  ...
  Future<dynamic> _request(
    ...
    final isMutation = method == 'POST' || method == 'PATCH' || method == 'DELETE';
    final fingerprint = isMutation ? '$method ${uri.toString()}\n${encodedBody ?? ''}' : null;
    final generatedKey = isMutation && idempotencyKey == null;
    final mutationKey = !isMutation ? null : idempotencyKey ?? _pendingMutationKeys.putIfAbsent(fingerprint!, LifeMateApiClient.createClientRequestId);
    ...
    for (var attempt = 1; attempt <= maxAttempts; attempt += 1) {
      final remainingMilliseconds = _retryBudget.inMilliseconds - budget.elapsedMilliseconds;
      if (remainingMilliseconds <= 0) {
        throw const LifeMateApiException(statusCode: 0, code: 'retry_budget_exhausted', message: 'LifeMate care management retry budget was exhausted.');
      }
      final attemptTimeout = Duration(milliseconds: min(_timeout.inMilliseconds, remainingMilliseconds));
      late final http.Response response;
      try {
        response = switch (method) {
          'GET' => await _http.get(uri, headers: headers).timeout(attemptTimeout),
          'POST' => await _http.post(uri, headers: headers, body: encodedBody).timeout(attemptTimeout),
          'PATCH' => await _http.patch(uri, headers: headers, body: encodedBody).timeout(attemptTimeout),
          'DELETE' => await _http.delete(uri, headers: headers, body: encodedBody).timeout(attemptTimeout),
          _ => throw ArgumentError.value(method, 'method'),
        };
      } on TimeoutException {
        if (attempt < maxAttempts && await _waitBeforeRetry(attempt, budget)) continue;
        throw const LifeMateApiException(statusCode: 0, code: 'network_timeout', message: 'LifeMate care management request timed out.');
      } on http.ClientException {
        if (attempt < maxAttempts && await _waitBeforeRetry(attempt, budget)) continue;
        throw const LifeMateApiException(statusCode: 0, code: 'network_unavailable', message: 'LifeMate care management service is unavailable.');
      }
      final retryResponse = _shouldRetryResponse(response);
      if (attempt < maxAttempts && retryResponse && await _waitBeforeRetry(attempt, budget, response: response)) continue;
      try {
        final decoded = _decode(response);
        if (generatedKey) _pendingMutationKeys.remove(fingerprint);
        return decoded;
      } on LifeMateApiException {
        if (generatedKey && !retryResponse) _pendingMutationKeys.remove(fingerprint);
        rethrow;
      }
    }
    throw StateError('LifeMate care management retry loop exited unexpectedly.');
  }

  Future<bool> _waitBeforeRetry(int attempt, Stopwatch budget, {http.Response? response}) async {
    var delay = _retryDelayForAttempt(attempt);
    final retryAfter = response == null ? null : int.tryParse(response.headers['retry-after'] ?? '');
    if (retryAfter != null && retryAfter > 0) {
      final serverDelay = Duration(seconds: retryAfter);
      if (serverDelay > delay) delay = serverDelay;
    }
    if (budget.elapsedMilliseconds + delay.inMilliseconds >= _retryBudget.inMilliseconds) return false;
  ...
  Duration _retryDelayForAttempt(int attempt) {
    ...
    return Duration(milliseconds: min(_retryMaxDelay.inMilliseconds, exponential + jitter));
  }
  ...
  bool _shouldRetryResponse(http.Response response) {
    ...
    try {
      ...
      return decoded is Map && decoded['code']?.toString() == 'idempotency_in_progress';
    } on FormatException {
      return false;
    }
  }
  ...
  dynamic _decode(http.Response response) {
    ...
    throw LifeMateApiException(
      ...
      message: problem['detail']?.toString() ?? problem['message']?.toString() ?? 'LifeMate care management request failed.',
    );
  }

  void close() { _pendingMutationKeys.clear(); _http.close(); }
  Uri _resolve(String path) { final basePath = _baseUri.path.replaceFirst(RegExp(r'/$'), ''); return _baseUri.replace(path: '$basePath$path'); }
  static Map<String, dynamic> _object(dynamic value) { if (value is! Map) throw const FormatException('LifeMate care management API returned a non-object payload.'); return Map<String, dynamic>.from(value); }
  static List<Map<String, dynamic>> _list(dynamic value) { if (value is! List) throw const FormatException('LifeMate care management API returned a non-list payload.'); return value.map((item) => Map<String, dynamic>.from(item as Map)).toList(growable: false); }
