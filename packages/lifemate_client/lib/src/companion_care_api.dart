...
class LifeMateCompanionCareApi {
  LifeMateCompanionCareApi({required Uri baseUri, required String? Function() accessToken, http.Client? httpClient})
      : _baseUri = baseUri,
        _accessToken = accessToken,
        _http = httpClient ?? http.Client();

  factory LifeMateCompanionCareApi.fromEnvironment({http.Client? httpClient}) {
    final config = AppConfig.fromEnvironment();
    return LifeMateCompanionCareApi(
  ...
  Future<Map<String, dynamic>> recordImpression({
    ...
    final uri = Uri.parse(
      '$base/api/v1/care/patients/$patientUserId/women-calendar/support-actions',
    );
    try {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        ...
        throw LifeMateApiException(
          ...
          message: problem['detail']?.toString() ??
              ...
              'Companion guidance impression failed.',
        );
      }
      if (decoded is! Map) {
        throw const FormatException(
          'Companion guidance response must be an object.',
        );
      }
      ...
      throw const LifeMateApiException(
        ...
        message: 'Companion guidance request timed out.',
      );
