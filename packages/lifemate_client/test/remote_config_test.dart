...
void main() {
  test('fresh persistent cache avoids unnecessary network request', () async {
    var networkCalls = 0;
    final client = LifeMateRemoteConfigClient(
      ...
      platform: 'android',
      accessToken: () => 'token',
      cacheRead: (_) async => jsonEncode(_snapshot()),
  ...
  test('stale cache is used on outage but protected flag fails closed after 24h', () async {
  ...
  test('fresh server snapshot replaces stale cache and preserves force update', () async {
    String? written;
    final client = LifeMateRemoteConfigClient(
      baseUri: Uri.parse('https://example.test'),
      product: 'wellmate',
  ...
  test('version presence contains no device fingerprint and is idempotency protected', () async {
