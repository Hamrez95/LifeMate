...
void main() {
  ...
  test('check-in and mood mutations bind the stable request id to idempotency', () async {
    ...
    final api = CocoonPregnancyDailyApiClient(
      ...
      httpClient: MockClient((request) async {
        ...
        final key = request.url.path.endsWith('/check-ins') ? 'checkIn' : 'mood';
        return http.Response(
          jsonEncode({'contractVersion': 1, key: capture(id: key)}),
          201,
        );
      }),
    );
    ...
    await api.createMood(
      clientRequestId: requestId,
      observedAtUtc: DateTime.utc(2026, 9, 12, 9),
      localDate: '2026-09-12',
      timeZone: 'Asia/Tehran',
      mood: CocoonPregnancyMood.good,
    );
