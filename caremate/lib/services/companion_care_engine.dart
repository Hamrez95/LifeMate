class CompanionGuidanceHistoryItem {
  const CompanionGuidanceHistoryItem({required this.guidanceId, required this.shownAtUtc});
  final String guidanceId;
  final DateTime shownAtUtc;
}

class CompanionSupportActionHistoryItem {
  const CompanionSupportActionHistoryItem({required this.actionType, required this.performedAtUtc});
  final String actionType;
  final DateTime performedAtUtc;
}

class CompanionCareGuidance {
  const CompanionCareGuidance({required this.id, required this.category, required this.contentVersion, required this.locale, required this.title, required this.message, required this.cooldown, this.supportActionType, this.supportActionLabel});
  final String id, category, contentVersion, locale, title, message;
  final Duration cooldown;
  final String? supportActionType, supportActionLabel;
}

/// Pure, deterministic and non-diagnostic guidance selector.
/// It accepts only fields already filtered by the server-side #109 scope gate.
class CompanionCareEngine {
  ...
  CompanionCareGuidance? select({required bool phaseAllowed,required bool wellbeingAllowed,required int? cycleDay,required String? mood,required int? energyLevel,required List<CompanionGuidanceHistoryItem> guidanceHistory,required List<CompanionSupportActionHistoryItem> supportActions,required String locale,required DateTime nowUtc}) {
