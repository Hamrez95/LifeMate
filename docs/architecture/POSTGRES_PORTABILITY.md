...
`postgres-portability` CI uses an unmodified `postgres:17.6-alpine` service container, applies the canonical baseline and every ordered migration, and verifies the expected LifeMate-owned schemas, restricted runtime roles and onboarding/recovery controls. A source policy scans canonical SQL and rejects direct references to provider-owned Auth/Storage/Realtime database schemas/helpers.
...
2. provision equivalent restricted runtime/migration/backup identities;
