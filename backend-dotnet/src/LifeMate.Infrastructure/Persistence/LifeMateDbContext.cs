...
public sealed class LifeMateDbContext : DbContext, IAppDbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<PrivacyConsent>(b => { b.ToTable("privacy_consents"); b.HasKey(x => x.Id); b.Property(x => x.UserId).HasColumnName("user_id"); b.HasOne<AppUser>().WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Restrict); b.Property(x => x.DocumentType).HasColumnName("document_type").HasConversion<string>().HasMaxLength(32); b.Property(x => x.DocumentVersion).HasColumnName("document_version").HasMaxLength(64); b.Property(x => x.GrantedAtUtc).HasColumnName("granted_at_utc"); b.Property(x => x.RevokedAtUtc).HasColumnName("revoked_at_utc"); b.Property(x => x.CreatedAtUtc).HasColumnName("created_at_utc"); b.HasIndex(x => new { x.UserId, x.DocumentType, x.DocumentVersion }); });
        modelBuilder.Entity<AuditLog>(b => { b.ToTable("audit_logs"); b.HasKey(x => x.Id); b.Property(x => x.ActorUserId).HasColumnName("actor_user_id"); b.HasOne<AppUser>().WithMany().HasForeignKey(x => x.ActorUserId).OnDelete(DeleteBehavior.SetNull); b.Property(x => x.Action).HasColumnName("action").HasMaxLength(128); b.Property(x => x.ResourceType).HasColumnName("resource_type").HasMaxLength(128); b.Property(x => x.ResourceId).HasColumnName("resource_id"); b.Property(x => x.MetadataJson).HasColumnName("metadata_json").HasColumnType("jsonb"); b.Property(x => x.CreatedAtUtc).HasColumnName("created_at_utc"); b.HasIndex(x => x.ActorUserId); b.HasIndex(x => new { x.ResourceType, x.ResourceId }); b.HasIndex(x => x.CreatedAtUtc); });
        modelBuilder.Entity<CareInvitation>(b => { b.ToTable("care_invitations"); b.HasKey(x => x.Id); b.Property(x => x.InviterUserId).HasColumnName("inviter_user_id"); b.HasOne<AppUser>().WithMany().HasForeignKey(x => x.InviterUserId).OnDelete(DeleteBehavior.Restrict); b.Property(x => x.ContactHash).HasColumnName("contact_hash").HasMaxLength(128).IsRequired(); b.Property(x => x.TokenHash).HasColumnName("token_hash").HasMaxLength(128).IsRequired(); b.HasIndex(x => x.TokenHash).IsUnique(); b.HasIndex(x => new { x.InviterUserId, x.ContactHash, x.Status }); b.Property(x => x.Status).HasColumnName("status").HasConversion<string>().HasMaxLength(32); b.Property(x => x.ExpiresAtUtc).HasColumnName("expires_at_utc"); b.Property(x => x.AcceptedAtUtc).HasColumnName("accepted_at_utc"); b.Property(x => x.RevokedAtUtc).HasColumnName("revoked_at_utc"); b.Property(x => x.CreatedAtUtc).HasColumnName("created_at_utc"); });
        modelBuilder.Entity<CareRelationship>(b => { b.ToTable("care_relationships"); b.HasKey(x => x.Id); b.Property(x => x.PatientUserId).HasColumnName("patient_user_id"); b.Property(x => x.CaregiverUserId).HasColumnName("caregiver_user_id"); b.HasOne<AppUser>().WithMany().HasForeignKey(x => x.PatientUserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<AppUser>().WithMany().HasForeignKey(x => x.CaregiverUserId).OnDelete(DeleteBehavior.Restrict); b.Property(x => x.Status).HasColumnName("status").HasConversion<string>().HasMaxLength(32); b.Property(x => x.ConsentedAtUtc).HasColumnName("consented_at_utc"); b.Property(x => x.RevokedAtUtc).HasColumnName("revoked_at_utc"); b.Property(x => x.CreatedAtUtc).HasColumnName("created_at_utc"); b.Property(x => x.UpdatedAtUtc).HasColumnName("updated_at_utc"); b.HasIndex(x => new { x.PatientUserId, x.CaregiverUserId, x.Status }).IsUnique(); b.HasIndex(x => x.CaregiverUserId); });
    }
}
