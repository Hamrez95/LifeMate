...
on conflict(code) do update set display_name=excluded.display_name,status='Active',is_system=true,updated_at_utc=now();
...
('commerce.entitlement.adjust.approve','commerce','ELEVATED',false,'Approve a manual entitlement adjustment'),
('commerce.entitlement.adjust.execute','commerce','ELEVATED',false,'Execute an approved manual entitlement adjustment')
...
cross join (values
  ...
  ('super_admin','commerce.entitlement.adjust.approve'),('super_admin','commerce.entitlement.adjust.execute')
) p(role_code,code)
...
on conflict(request_type) do update set
  ...
  self_approval_allowed=false,status='Active',updated_at_utc=now(),version=admin.approval_policies.version+1;
...
grant select,insert on commerce.entitlement_adjustments to lifemate_admin_runtime;
create policy entitlement_adjustments_admin_runtime on commerce.entitlement_adjustments
  for select to lifemate_admin_runtime using (true);
create policy entitlement_adjustments_admin_insert on commerce.entitlement_adjustments
...
as $$
  with tf as (
    ...
    select f.id as feature_id,f.code,
      count(e.id) filter (
        where e.status='Active' and e.starts_at_utc<=now()
  ...
  select jsonb_build_object(
    ...
    'features',coalesce(jsonb_agg(jsonb_build_object(
      ...
      'hasIndefinite',coalesce(has_indefinite,false),'maxExpiresAtUtc',max_expires_at_utc
    ) order by code),'[]'::jsonb)
  ) from rows
$$;
...
language plpgsql
stable
set search_path=commerce,identity,pg_temp
as $$
declare
  v_before jsonb;
  v_after jsonb;
...
begin
  ...
  select
    coalesce(sum((x->>'activeCount')::bigint),0),
    coalesce(bool_or((x->>'hasIndefinite')::boolean),false),
    max(nullif(x->>'maxExpiresAtUtc','')::timestamptz)
  ...
  if p_action='Revoke' then
    ...
    if v_active_count=0 then
      return jsonb_build_object('httpStatus',409,'code','entitlement_nothing_to_reduce','message','No active entitlement is available to reduce.');
    end if;
    if not v_has_indefinite and (v_max_expiry is null or p_exact_expires_at_utc>=v_max_expiry) then
      return jsonb_build_object('httpStatus',409,'code','entitlement_reduce_not_smaller','message','Reduced expiry must be earlier than the current effective expiry.');
    end if;
  ...
  if p_action='Extend' and v_active_count=0 then
    return jsonb_build_object('httpStatus',409,'code','entitlement_nothing_to_extend','message','No active entitlement is available to extend.');
  end if;
  if p_action='Extend' and v_max_expiry is not null and v_result_expiry<=v_max_expiry then
    return jsonb_build_object('httpStatus',409,'code','entitlement_extend_not_larger','message','Extended expiry must be later than the current effective expiry.');
  end if;

  v_after:=jsonb_build_object(
    'accountId',p_account_id,'targetType',p_target_type,'targetId',p_target_id,'action',p_action,
    'effectiveExpiresAtUtc',v_result_expiry,'featureCount',v_feature_count
  );
...
language plpgsql
set search_path=commerce,admin,security,pg_temp
as $$
declare
...
begin
  ...
  v_before:=v_preview->'before'; v_delta:=v_preview->'delta'; v_after:=v_preview->'after';
  ...
  if v_abuse->>'action'='Deny' then
    return jsonb_build_object('httpStatus',403,'code','entitlement_adjustment_abuse_denied','message','Adjustment was denied by an explainable abuse rule.','abuseDecisionId',v_abuse->>'decisionId','reasonCodes',v_abuse->'reasonCodes');
  end if;
  ...
  if (not v_founder) or v_abuse->>'action'='RequireApproval' then
    if p_approval_request_id is null or p_approval_expected_version is null then
      return jsonb_build_object('httpStatus',409,'code','entitlement_adjustment_approval_required','message','An approved adjustment request is required.','approvalRequestType',coalesce(v_abuse->>'approvalRequestType','commerce.entitlement.adjustment'));
    end if;
    ...
    if v_abuse->>'action'='RequireApproval' and coalesce(v_abuse->>'approvalRequestType','commerce.entitlement.adjustment')<>'commerce.entitlement.adjustment' then
  ...
  if p_action='Grant' then
    ...
    v_event_type:=case p_action when 'Extend' then 'Renewed' when 'Reduce' then 'Cancelled' else 'Revoked' end;
    for v_entitlement in
      select e.* from commerce.entitlements e
      join commerce.entitlement_adjustment_target_features(p_target_type,p_target_id) tf on tf.feature_id=e.feature_id
      where e.grantee_account_id=p_account_id and e.status='Active' and e.starts_at_utc<=now()
        and (e.expires_at_utc is null or e.expires_at_utc>now())
      for update of e
    loop
      if p_action='Extend' then
        update commerce.entitlements set expires_at_utc=(v_after->>'effectiveExpiresAtUtc')::timestamptz,updated_at_utc=now()
        where id=v_entitlement.id and expires_at_utc is not null and expires_at_utc<(v_after->>'effectiveExpiresAtUtc')::timestamptz;
      elsif p_action='Reduce' then
        update commerce.entitlements set expires_at_utc=(v_after->>'effectiveExpiresAtUtc')::timestamptz,updated_at_utc=now()
        where id=v_entitlement.id and (expires_at_utc is null or expires_at_utc>(v_after->>'effectiveExpiresAtUtc')::timestamptz);
      else
        update commerce.entitlements set status='Revoked',expires_at_utc=least(coalesce(expires_at_utc,now()),now()),updated_at_utc=now()
  ...
  values(p_actor_account_id,'commerce.entitlement.adjust.execute','entitlement_adjustment',v_adjustment_id::text,'Succeeded',trim(p_reason),p_correlation_id,p_idempotency_key,true,
  ...
  update admin.idempotency_keys set status='Completed',response_status=200,response_json=v_result,updated_at_utc=now()
...
revoke all on function commerce.entitlement_adjustment_target_features(character varying,uuid) from public;
revoke all on function commerce.entitlement_adjustment_snapshot(uuid,character varying,uuid) from public;
revoke all on function commerce.preview_entitlement_adjustment(uuid,character varying,uuid,character varying,character varying,integer,timestamptz) from public;
revoke all on function commerce.execute_entitlement_adjustment(uuid,uuid,character varying,uuid,character varying,character varying,integer,timestamptz,character varying,uuid,bigint,uuid,character varying,character varying) from public;
...
grant execute on function commerce.execute_entitlement_adjustment(uuid,uuid,character varying,uuid,character varying,character varying,integer,timestamptz,character varying,uuid,bigint,uuid,character varying,character varying) to lifemate_admin_runtime;
