...
create or replace function analytics.research_json_contains_direct_identifier(p_value jsonb)
returns boolean
language plpgsql
immutable
...
begin
  ...
  if jsonb_typeof(p_value)='object' then
    for v_key,v_child in select key,value from jsonb_each(p_value) loop
      v_compact := lower(regexp_replace(v_key,'[^a-zA-Z0-9]','','g'));
      if v_compact = any(array[
        'email','emailaddress','phone','phonenumber','mobile','mobilenumber','name','fullname',
        'address','nationalid','contact','contactpoint','token','externalid','personid','accountid',
        'appuserid','authuserid','userid'
      ]) then return true; end if;
      if lower(v_key) = any(array['field','attribute','column','key']) and jsonb_typeof(v_child)='string' then
        v_selector := lower(regexp_replace(trim(both '"' from v_child::text),'[^a-zA-Z0-9]','','g'));
        if v_selector = any(array[
          'email','emailaddress','phone','phonenumber','mobile','mobilenumber','name','fullname',
          'address','nationalid','contact','contactpoint','token','externalid','personid','accountid',
          'appuserid','authuserid','userid'
        ]) then return true; end if;
      end if;
      if analytics.research_json_contains_direct_identifier(v_child) then return true; end if;
    end loop;
...
create table if not exists analytics.dataset_definitions (
  ...
  filter_json jsonb not null default '{}'::jsonb check (
    jsonb_typeof(filter_json)='object'
    and octet_length(filter_json::text)<=16000
    and not analytics.research_json_contains_direct_identifier(filter_json)
  ),
...
create table if not exists analytics.dataset_privacy_policies (
  ...
  quasi_identifier_rules jsonb not null default '{}'::jsonb check (
    jsonb_typeof(quasi_identifier_rules)='object'
    and octet_length(quasi_identifier_rules::text)<=16000
    and not analytics.research_json_contains_direct_identifier(quasi_identifier_rules)
  ),
...
$$;
revoke all on function admin.account_is_active_founder(uuid,timestamptz) from public;
...
begin
  ...
  if jsonb_typeof(coalesce(p_filter,'{}'::jsonb)) <> 'object' or octet_length(coalesce(p_filter,'{}'::jsonb)::text)>16000
  ...
  if jsonb_typeof(coalesce(p_quasi_identifier_rules,'{}'::jsonb)) <> 'object' or octet_length(coalesce(p_quasi_identifier_rules,'{}'::jsonb)::text)>16000
...
revoke all on function analytics.research_json_contains_direct_identifier(jsonb) from public;
