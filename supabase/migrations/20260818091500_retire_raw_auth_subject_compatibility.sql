-- #356 / #217: prepare the compatibility schema for protected raw Auth-subject
-- retirement. Runtime activation remains fail-closed and disabled by default.
-- A scrubbed AppUser must keep its Account/Self Person compatibility projection
-- without recreating a plaintext external identity.

alter table lifemate.app_users
  alter column auth_subject drop not null;

comment on column lifemate.app_users.auth_subject is
  'Temporary legacy Supabase Auth compatibility subject. Nullable after protected token-only retirement; canonical lookup uses externally keyed identity tokens.';
...
begin
  ...
  -- Raw identity compatibility is intentionally conditional. Once protected
  -- retirement nulls auth_subject, the trigger must never recreate the raw
  -- supabase_auth/supabase row from a later status/timestamp update.
  if new.auth_subject is not null then
