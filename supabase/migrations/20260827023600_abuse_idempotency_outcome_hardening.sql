...
as $$
declare
  v_operation constant character varying(120):='security.abuse.rule.upsert';
...
begin
  ...
  if found then
    if v_existing.request_hash<>p_request_hash then
      return jsonb_build_object('httpStatus',409,'code','idempotency_conflict','message','This Idempotency-Key was already used for a different rule mutation.','replayed',false);
    end if;
    ...
    return jsonb_build_object('httpStatus',409,'code','idempotency_in_progress','message','The matching rule mutation is still processing.','replayed',false);
  end if;
  ...
  v_status:=coalesce((v_result->>'httpStatus')::integer,500);

  update admin.idempotency_keys
  set status='Completed',response_status=v_status,response_json=v_result,updated_at_utc=now()
...
as $$
declare
  v_operation constant character varying(120):='security.abuse.rule.retire';
  v_existing admin.idempotency_keys%rowtype;
  v_result jsonb;
  v_status integer;
begin
  ...
  if found then
    if v_existing.request_hash<>p_request_hash then
      return jsonb_build_object('httpStatus',409,'code','idempotency_conflict','message','This Idempotency-Key was already used for a different retirement.','replayed',false);
    end if;
    if v_existing.status='Completed' and v_existing.response_json is not null then
      return v_existing.response_json||jsonb_build_object('replayed',true);
    end if;
    return jsonb_build_object('httpStatus',409,'code','idempotency_in_progress','message','The matching retirement is still processing.','replayed',false);
  end if;
  insert into admin.idempotency_keys(actor_account_id,operation,idempotency_key,request_hash,status)
  values(p_actor_account_id,v_operation,p_idempotency_key,p_request_hash,'Processing');
...
revoke all on function security.retire_abuse_rule_idempotent(uuid,uuid,bigint,character varying,uuid,character varying,character varying) from public,anon,authenticated;
