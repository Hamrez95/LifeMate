...
alter table security.abuse_decisions
  add column if not exists subject_account_id uuid;
update security.abuse_decisions
set subject_account_id=actor_account_id
where subject_account_id is null;
alter table security.abuse_decisions
  alter column subject_account_id set not null;
create index if not exists ix_security_abuse_decisions_subject
  on security.abuse_decisions(subject_account_id,context_code,evaluated_at_utc desc);
...
begin
  if p_actor_account_id is null or p_subject_account_id is null then
    return jsonb_build_object('httpStatus',400,'code','abuse_subject_invalid','message','Actor and subject accounts are required.');
  end if;
  ...
  if found then
    if v_existing.request_hash<>p_request_hash or v_existing.subject_account_id<>p_subject_account_id then
  ...
  loop
    v_subject_hash:=security.abuse_subject_key_hash(p_subject_account_id,v_rule.subject_scope);
    v_triggered:=false;
  ...
  insert into security.abuse_decisions(
    actor_account_id,subject_account_id,context_code,operation_key_hash,final_action,matched_rule_ids,reason_codes,
    approval_request_type,rule_set_hash,idempotency_key,request_hash
  ) values(
    p_actor_account_id,p_subject_account_id,v_context,v_operation_hash,v_final,v_rule_ids,v_reasons,v_approval,
...
revoke all on function security.evaluate_abuse_rules(uuid,uuid,character varying,character varying,character varying[],character varying,character varying) from public;
grant execute on function security.evaluate_abuse_rules(uuid,uuid,character varying,character varying,character varying[],character varying,character varying) to lifemate_edge_runtime,lifemate_admin_runtime;
...
drop function security.evaluate_abuse_rules(uuid,character varying,character varying,character varying[],character varying,character varying);
