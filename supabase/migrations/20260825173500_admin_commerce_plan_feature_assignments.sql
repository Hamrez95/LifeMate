...
-- Explicit plan-to-feature assignments are separate from product-level feature
-- catalog membership and from account/person entitlement grants.
...
drop policy if exists plan_feature_assignments_no_direct_access
  on commerce.plan_feature_assignments;
...
grant execute on function admin.get_commerce_plan_features(uuid)
...
begin
  ...
  if p_plan_id is null or p_feature_id is null or p_assigned is null
     or p_expected_version is null or p_expected_version < 0 then
    return jsonb_build_object(
      'httpStatus',400,
      'code','plan_feature_invalid',
      'message','Plan feature fields are invalid.',
      'replayed',false
    );
  end if;
  ...
  if p_reason is null or length(trim(p_reason)) not between 10 and 1000
     ...
     or length(p_idempotency_key) not between 8 and 180
     or p_request_hash is null
     or length(p_request_hash) not between 32 and 128 then
    return jsonb_build_object(
      'httpStatus',400,
      'code','plan_feature_request_invalid',
      'message','Plan feature request metadata is invalid.',
      'replayed',false
    );
  end if;
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(
      'commerce.plan_feature:' || p_plan_id::text || ':' || p_feature_id::text,
      0
    )
  );
  ...
  if not exists (
    ...
    join commerce.product_features pf
      on pf.product_id = pl.product_id and pf.feature_id = p_feature_id
    where pl.id = p_plan_id
    ...
    v_response := jsonb_build_object(
      'httpStatus',409,
      'code','plan_feature_not_assignable',
      'message','The feature is not assignable to this active plan.',
      'replayed',false
    );
  else
    select * into v_assignment
    from commerce.plan_feature_assignments
    ...
    if not found then
      if p_expected_version <> 0 then
        v_response := jsonb_build_object(
          'httpStatus',409,
          'code','plan_feature_version_conflict',
          'message','Plan feature version does not match.',
          'replayed',false
        );
      else
        insert into commerce.plan_feature_assignments(
          plan_id, feature_id, assigned, version
        ) values (
          p_plan_id, p_feature_id, p_assigned, 1
        )
        ...
        v_response := jsonb_build_object(
          ...
          'assigned',v_assignment.assigned,
  ...
  insert into admin.audit_events(
    ...
    p_actor_account_id,
    v_operation,
    'commerce_plan_feature',
    p_plan_id::text || ':' || p_feature_id::text,
    case when (v_response->>'httpStatus')::integer < 400
