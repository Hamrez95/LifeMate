...
begin
  ...
  -- A different Idempotency-Key must not re-run abuse accounting or create a
  -- second logical reward for the same source/rule/beneficiary tuple.
  if v_response is null then
    ...
    if found then
      v_response:=jsonb_build_object(
        'httpStatus',200,'code','ok','rewardEventId',v_event.id,
        'rewardKind',v_event.reward_kind,'status',v_event.status,
        'ruleVersion',v_event.reward_rule_version,'replayed',true
      );
    end if;
  end if;
