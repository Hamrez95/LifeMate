...
create index if not exists feedback_items_owner_created_idx on feedback.items (app_user_id, created_at desc);
create index if not exists feedback_items_queue_idx on feedback.items (status, kind, created_at desc);
create index if not exists feedback_items_product_idx on feedback.items (product_code, created_at desc);
...
create or replace view admin.product_feedback_queue_v1 with (security_invoker = true) as
select id, kind::text as kind, status::text as status, product_code, app_version, build_number,
       nps_score, advocacy_opt_in, linked_support_ticket_id, linked_product_issue_ref,
       created_at, acknowledged_at, triaged_at, resolved_at
from feedback.items;
revoke all on admin.product_feedback_queue_v1 from public, anon, authenticated;

create or replace view admin.product_feedback_trends_v1 with (security_invoker = true) as
select date_trunc('day', created_at) as day, product_code, kind::text as kind,
