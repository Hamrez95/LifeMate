    ...
    select p.code as product_code,
      jsonb_build_object(
        ...
        'entitlements',coalesce((
          select jsonb_agg(jsonb_build_object('feature',f.code,'source',e.source,'status',e.status,
            'startsAtUtc',e.starts_at_utc,'expiresAtUtc',e.expires_at_utc) order by f.code)
          from commerce.entitlements e join commerce.features f on f.id=e.feature_id
          where e.grantee_account_id=v_account and e.status='Active'
            and e.starts_at_utc<=v_now and (e.expires_at_utc is null or e.expires_at_utc>v_now)
        ),'[]'::jsonb)
      ) as product_payload
    from commerce.products p
