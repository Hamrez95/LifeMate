...
-- ADM-COM-005 intentionally reuses the canonical commerce.promo.write capability
-- defined by the Admin control-plane foundation. Promotion rules remain separate
-- from Plans, Entitlements, Transactions and payment-provider state.
...
comment on table commerce.promotions is
  'Commercial promotion rules. A Promotion is independent from Plan, Entitlement, Transaction and Provider Event state.';
comment on table commerce.discount_codes is
  'Redeemable codes attached to Promotions. No payment credentials or provider references are stored here.';
...
begin
  if not admin.account_has_permission(p_actor_account_id, 'commerce.promo.write') then
  ...
  if p_max_redemptions is not null and p_max_redemptions <= 0 then
    return jsonb_build_object('httpStatus', 400, 'code', 'promotion_limit_invalid', 'message', 'Promotion redemption limit is invalid.', 'replayed', false);
  end if;
  if p_code_max_redemptions is not null and p_code_max_redemptions <= 0 then
    return jsonb_build_object('httpStatus', 400, 'code', 'promotion_limit_invalid', 'message', 'Discount-code redemption limit is invalid.', 'replayed', false);
  end if;
  if p_max_redemptions is not null and p_code_max_redemptions is not null and p_code_max_redemptions > p_max_redemptions then
    return jsonb_build_object('httpStatus', 400, 'code', 'promotion_limit_invalid', 'message', 'Discount-code limit cannot exceed the promotion limit.', 'replayed', false);
  end if;

  if p_discount_type = 'Percentage' then
    if p_percentage_basis_points is null
       or p_percentage_basis_points < 1
       or p_percentage_basis_points > 10000
       or p_fixed_amount_minor is not null
    ...
    if p_fixed_amount_minor is null
       or p_fixed_amount_minor <= 0
       or p_currency is null
       or p_currency !~ '^[A-Z]{3}$'
       or p_percentage_basis_points is not null then
  ...
  if p_idempotency_key is null
     ...
     or length(p_request_hash) < 32
     or length(p_request_hash) > 128 then
  ...
  -- Serialize all mutation workflows that target the same human-visible code.
  ...
  if p_product_id is not null and not exists(select 1 from commerce.products where id = p_product_id) then
    ...
    if v_code_id is null then
      delete from commerce.promotions where id = v_promotion_id;
      v_promotion_id := null;
      v_response := jsonb_build_object('httpStatus', 409, 'code', 'discount_code_conflict', 'message', 'Discount code already exists.', 'replayed', false);
    else
...
create or replace function admin.update_commerce_promotion(
  p_actor_account_id uuid,
  p_promotion_id uuid,
  p_product_id uuid,
  p_name character varying,
...
as $$
declare
  v_operation constant character varying := 'commerce.promotion.update';
  v_existing admin.idempotency_keys%rowtype;
...
begin
  ...
  if v_code is null or v_code !~ '^[A-Z0-9][A-Z0-9._-]{2,63}$' or p_code_status not in ('Active', 'Disabled') then
    return jsonb_build_object('httpStatus', 400, 'code', 'discount_code_invalid', 'message', 'Discount code configuration is invalid.', 'replayed', false);
  end if;
  if p_starts_at_utc is null or (p_ends_at_utc is not null and p_ends_at_utc <= p_starts_at_utc) then
    return jsonb_build_object('httpStatus', 400, 'code', 'promotion_window_invalid', 'message', 'Promotion time window is invalid.', 'replayed', false);
  end if;
  ...
  if not found then
  ...
  elsif v_previous_status = 'Expired' or exists(
    select 1 from commerce.promotions where id = p_promotion_id and ends_at_utc is not null and ends_at_utc <= now()
  ) then
    update commerce.promotions set status = 'Expired', updated_at_utc = now() where id = p_promotion_id;
    v_response := jsonb_build_object('httpStatus', 409, 'code', 'promotion_expired', 'message', 'Expired promotion rules cannot be edited.', 'replayed', false);
  elsif v_previous_status = 'Active' then
    v_response := jsonb_build_object('httpStatus', 409, 'code', 'promotion_must_be_paused', 'message', 'Pause the promotion before editing financial rules.', 'replayed', false);
  elsif p_product_id is not null and not exists(select 1 from commerce.products where id = p_product_id) then
    v_response := jsonb_build_object('httpStatus', 404, 'code', 'commerce_product_not_found', 'message', 'Commerce product was not found.', 'replayed', false);
  else
    ...
    if v_code_id is null then
      v_response := jsonb_build_object('httpStatus', 409, 'code', 'promotion_code_missing', 'message', 'Promotion has no editable primary code.', 'replayed', false);
    else
      perform pg_advisory_xact_lock(hashtextextended('commerce.discount_code:' || lower(v_code), 0));
      if exists(
        select 1 from commerce.discount_codes
        where lower(code) = lower(v_code) and id <> v_code_id
      ) then
        v_response := jsonb_build_object('httpStatus', 409, 'code', 'discount_code_conflict', 'message', 'Discount code already exists.', 'replayed', false);
      else
        ...
        set code = v_code,
            ...
            max_redemptions = p_code_max_redemptions,
            updated_at_utc = now()
        where id = v_code_id;
        ...
        insert into admin.audit_events(
          ...
          p_actor_account_id, 'commerce.promotion.update', 'commerce_promotion', p_promotion_id::text,
          'Succeeded', trim(p_reason), p_correlation_id, p_idempotency_key, false,
          jsonb_build_object(
            'status', v_previous_status,
            'discountType', p_discount_type,
            'discountCodeId', v_code_id,
            'codeStatus', p_code_status
          )
        );
  ...
  if (v_response->>'httpStatus')::integer >= 400 then
    insert into admin.audit_events(
      ...
      p_actor_account_id, 'commerce.promotion.update', 'commerce_promotion', p_promotion_id::text,
      'Denied', coalesce(v_response->>'message', 'Promotion update denied'), p_correlation_id,
      p_idempotency_key, false, jsonb_build_object('code', v_response->>'code')
    );
  end if;
...
begin
  ...
  select status, ends_at_utc into v_previous_status, v_ends_at_utc
  from commerce.promotions
  where id = p_promotion_id
  for update;
  ...
  if not found then
    v_response := jsonb_build_object('httpStatus', 404, 'code', 'commerce_promotion_not_found', 'message', 'Promotion was not found.', 'replayed', false);
  elsif v_previous_status = 'Expired' or (v_ends_at_utc is not null and v_ends_at_utc <= now()) then
    update commerce.promotions set status = 'Expired', updated_at_utc = now() where id = p_promotion_id;
    ...
    insert into admin.audit_events(
      ...
      'Succeeded', trim(p_reason), p_correlation_id, p_idempotency_key, false,
      jsonb_build_object('previousStatus', v_previous_status, 'status', p_target_status, 'noop', true)
    );
    v_response := jsonb_build_object('httpStatus', 200, 'code', 'ok', 'promotionId', p_promotion_id, 'previousStatus', v_previous_status, 'status', p_target_status, 'noop', true, 'replayed', false);
  else
    update commerce.promotions
    ...
    insert into admin.audit_events(
      ...
      p_actor_account_id, 'commerce.promotion.status.change', 'commerce_promotion', p_promotion_id::text,
      'Succeeded', trim(p_reason), p_correlation_id, p_idempotency_key, false,
      jsonb_build_object('previousStatus', v_previous_status, 'status', p_target_status, 'noop', false)
    );

    v_response := jsonb_build_object('httpStatus', 200, 'code', 'ok', 'promotionId', p_promotion_id, 'previousStatus', v_previous_status, 'status', p_target_status, 'noop', false, 'replayed', false);
  end if;
  ...
  if (v_response->>'httpStatus')::integer >= 400 then
    insert into admin.audit_events(
      ...
      jsonb_build_object('targetStatus', p_target_status, 'code', v_response->>'code')
    );
  end if;
...
revoke all on function admin.update_commerce_promotion(
  uuid, uuid, uuid, character varying, character varying, text, integer, bigint, text,
  timestamptz, timestamptz, integer, character varying, text, integer, character varying,
  uuid, character varying, character varying
) from public;
...
grant execute on function admin.update_commerce_promotion(
  uuid, uuid, uuid, character varying, character varying, text, integer, bigint, text,
  timestamptz, timestamptz, integer, character varying, text, integer, character varying,
  uuid, character varying, character varying
) to lifemate_admin_runtime;
...
comment on function admin.update_commerce_promotion(
  uuid, uuid, uuid, character varying, character varying, text, integer, bigint, text,
  timestamptz, timestamptz, integer, character varying, text, integer, character varying,
  uuid, character varying, character varying
) is 'Audited, idempotent financial-rule update for a Draft or Paused Promotion and its primary Discount Code.';
comment on function admin.set_commerce_promotion_status(
  uuid, uuid, text, character varying, uuid, character varying, character varying
) is 'Audited, idempotent Promotion lifecycle transition restricted to Active/Paused; no-op requests are audited.';
