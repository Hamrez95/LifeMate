...
export function readMessagingTokenKeySet(
  ...
  const hasVersion = previousVersion.length > 0;
  if (hasSecret !== hasVersion) {
    throw new Error("Previous messaging token key and version must be configured together.");
  }
  ...
  if (previous.keyVersion === active.keyVersion) {
    throw new Error("Previous messaging token key version must differ from active key version.");
  }
  return { active, previous };
}

export function messagingTokenKeyForVersion(
  keys: MessagingTokenKeySet,
  version: number,
): MessagingTokenKey | null {
