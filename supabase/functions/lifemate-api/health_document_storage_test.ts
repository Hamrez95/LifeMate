...
Deno.test(
  "health document validation enforces allowed content types and bytes",
  () => {
    assertEquals(
...
Deno.test(
  "Health Record Storage uses a private opaque object contract",
  async () => {
    const source = await Deno.readTextFile(
      new URL("./health_document_storage.ts", import.meta.url),
    );
    assertEquals(
      source.includes('healthDocumentBucket = "health-records"'),
      true,
    );
    ...
    assertEquals(
      source.includes("healthDocumentSignedUrlLifetimeSeconds"),
      true,
    );
  },
);
