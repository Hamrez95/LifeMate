...
export function createFeedbackRouteHandler(databaseUrl: string) {
  ...
  return async function feedbackRouteHandler(input: { request: Request; path: string; appUserId: string; }): Promise<Response | null> {
    ...
    if ("attachment" in body || "attachmentUrl" in body || "file" in body) {
      throw new ApiError(400, "feedback_attachment_path_required", "Feedback attachments must use the reviewed secure attachment flow.");
    }
    const submitted = await store.submit(appUserId, parseFeedbackSubmission(body), requireMutationIdempotencyKey(request));
