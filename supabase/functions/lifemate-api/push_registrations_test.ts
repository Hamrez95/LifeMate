...
Deno.test("push registration accepts bounded FCM token metadata", () => {
  assertEquals(parsePushRegistration({
...
Deno.test("push registration rejects short token", async () => {
  await assertRejects(async () => {
    try {
      parsePushRegistration({ productCode: "wellmate", platform: "Android", provider: "fcm", token: "short" });
    } catch (error) {
      if (error instanceof ApiError) assertEquals(error.code, "push_registration_invalid");
      throw error;
    }
  }, ApiError);
});
...
Deno.test("push store never persists plaintext token", async () => {
  const source = await Deno.readTextFile(new URL("./push_registrations.ts", import.meta.url));
