...
import { type createSupportAttachmentRuntime, createSupportAttachmentRuntimeFromEnvironment } from "./support_attachment_storage.ts";
...
export function createGrowthRouteHandler(databaseUrl: string, contactHashingSecret: string, supportAttachments?: AttachmentRuntime) {
  ...
  const supportRoutes = createSupportConversationRouteHandler(createSupportConversationStore(databaseUrl), supportAttachments ?? createSupportAttachmentRuntimeFromEnvironment());
  return async function growthRouteHandler(input: { request: Request; path: string; appUserId: string }): Promise<Response | null> {
    ...
    const feedbackResponse = await feedbackRoutes(input); if (feedbackResponse) return feedbackResponse;
    const supportResponse = await supportRoutes(input); if (supportResponse) return supportResponse;
    const { request, path, appUserId } = input;
    if (request.method === "POST" && path === "/api/v1/growth/gifts") { enforceRateLimit(`growth-gift:${appUserId}`, 10, 60 * 60_000); return json(await store.createGift({ appUserId, body: await readJsonObject(request), idempotencyKey: requireMutationIdempotencyKey(request) }), 201); }
    if (request.method === "POST" && path === "/api/v1/growth/referral-code") { enforceRateLimit(`growth-referral-code:${appUserId}`, 5, 24 * 60 * 60_000); return json(await store.ensureReferralCode(appUserId)); }
    if (request.method === "POST" && path === "/api/v1/growth/referrals/attribute") { enforceRateLimit(`growth-referral-attribute:${appUserId}`, 5, 24 * 60 * 60_000); return json(await store.attributeReferral({ appUserId, body: await readJsonObject(request), idempotencyKey: requireMutationIdempotencyKey(request) }), 201); }
    if (request.method === "POST" && path === "/api/v1/growth/advocacy-submissions") { enforceRateLimit(`growth-advocacy:${appUserId}`, 10, 24 * 60 * 60_000); return json(await store.submitAdvocacy({ appUserId, body: await readJsonObject(request), idempotencyKey: requireMutationIdempotencyKey(request) }), 201); }
    if (request.method === "GET" && path === "/api/v1/growth/rewards") return json({ items: await store.listRewards(appUserId) });
    return null;
  };
}