  ...
  return async (input: {
    ...
    if (request.method === "POST" && path === "/api/v1/support/conversations") {
      ...
      return json(await store.open(appUserId, {
        productCode: optionalCode(body.productCode, 64),
        category: optionalCode(body.category, 64) ?? "general",
        body: requiredMessage(body.body),
        clientMessageId: requiredUuid(body.clientMessageId),
      }), 201);
    }

    const match = path.match(/^\/api\/v1\/support\/conversations\/([0-9a-f-]{36})(?:\/(messages|read))?$/i);
    ...
    if (request.method === "GET" && suffix === null) {
      ...
      return json({ items: await store.list(appUserId, ticketId, beforeAt, limit) });
    }
...
function optionalCode(value: unknown, max: number): string | null {
  if (value == null || value === "") return null;
  if (typeof value !== "string") throw new ApiError(400, "support_code_invalid", "Support code is invalid.");
  const result = value.trim().toLowerCase();
  if (!result || result.length > max || !/^[a-z0-9][a-z0-9_.:-]*$/.test(result)) {
    throw new ApiError(400, "support_code_invalid", "Support code is invalid.");
  }
  return result;
}
