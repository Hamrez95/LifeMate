...
Deno.test(
  "profile photo storage creates a private bucket when hosted Storage returns 400 for missing bucket",
  async () => {
    ...
    globalThis.fetch = ((input: string | URL | Request, init?: RequestInit) => {
      ...
      if (
        ...
        return Promise.resolve(
          new Response('{"message":"Bucket not found"}', { status: 400 }),
        );
      }
      ...
      if (
        method === "POST" && url.includes("/storage/v1/object/profile-photos/")
      ) {
        return Promise.resolve(new Response("{}", { status: 200 }));
      }
