...
Deno.test("Health Connect measurement normalizes with measured provenance", () => {
  assertEquals(normalizeWearableMeasurement({
...
Deno.test("malformed or unsupported units fail closed", () => {
  assertThrows(() => normalizeWearableMeasurement({
    platform: "android_health_connect",
...
Deno.test("collection requires connected, granted and exact measurement permission", () => {
  ...
  assertEquals(wearableCollectionAllowed({ ...base, permissionState: "revoked" }, "sleep_duration"), false);
  assertEquals(wearableCollectionAllowed({ ...base, connectionStatus: "disconnected" }, "sleep_duration"), false);
});

Deno.test("revoke/disconnect stops future derived personalized output", () => {
  assertEquals(wearableDerivedOutputAllowed({ connectionStatus: "connected", permissionState: "granted" }), true);
  assertEquals(wearableDerivedOutputAllowed({ connectionStatus: "connected", permissionState: "revoked" }), false);
  assertEquals(wearableDerivedOutputAllowed({ connectionStatus: "disconnected", permissionState: "granted" }), false);
});
