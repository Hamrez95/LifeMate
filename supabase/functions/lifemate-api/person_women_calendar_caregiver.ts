...
const guidanceIdPattern = /^[a-z0-9][a-z0-9_.-]{0,79}$/;
const contentVersionPattern = /^[a-z0-9][a-z0-9_.-]{0,39}$/;
...
export function createPersonWomenCalendarCaregiverStore(databaseUrl: string) {
  ...
  async function getCareSummary(
    ...
    const guidanceHistory = await sql`
      ...
      where patient_person_id=${patientPersonId}::uuid
        ...
        and relationship_id=${relationshipId}::uuid
      order by shown_at_utc desc,id limit 30
    `;
    ...
    return {
      patient: { displayName: patientProfile?.display_name ?? "LifeMate User", avatarKey: patientProfile?.avatar_key ?? "person_purple" },
      ...
      supportActions: privacy.viewSharedWellbeing ? actions.map((row: Row) => ({
        actionType: String(row.action_type).toLowerCase(), performedAtUtc: iso(row.performed_at_utc),
      })) : [],
      guidanceHistory: guidanceHistory.map((row: Row) => ({
        guidanceId: String(row.guidance_id), contentVersion: String(row.content_version),
        category: String(row.category), shownAtUtc: iso(row.shown_at_utc),
      })),
    };
  }
  ...
  async function recordCareSupportAction(
    ...
    const { caregiverPersonId, patientPersonId } = await resolveCarePeople(sql, caregiverAppUserId, patientAppUserId);
    const relationshipRows = await sql`
      select r.id, s.* from lifemate.care_relationships r
      left join lifemate.women_companion_privacy_scopes s on s.relationship_id = r.id
      where r.patient_person_id=${patientPersonId}::uuid
        and r.caregiver_person_id=${caregiverPersonId}::uuid
    ...
    if (normalized === "guidance_impression") {
      const guidanceId = String(body.guidanceId ?? "").trim().toLowerCase();
      const contentVersion = String(body.contentVersion ?? "").trim().toLowerCase();
      const category = String(body.category ?? "").trim().toLowerCase();
      if (!guidanceIdPattern.test(guidanceId) || !contentVersionPattern.test(contentVersion) || !guidanceCategories.has(category)) {
        throw new ApiError(400, "invalid_companion_guidance_impression", "Guidance impression metadata is invalid.");
      }
      const categoryAllowed = category === "general" ||
        (category === "phase" && privacy.viewPhaseSummary) ||
        ((category === "mood" || category === "energy") && privacy.viewSharedWellbeing);
      if (!categoryAllowed) throw accessDenied();
      const recent = await sql`
        select id from lifemate.women_companion_guidance_history
        where relationship_id=${relationshipId}::uuid
          and caregiver_person_id=${caregiverPersonId}::uuid
          and guidance_id=${guidanceId}
          and shown_at_utc >= now() - interval '10 minutes'
        limit 1
      `;
      if (recent[0]) return { id: recent[0].id, guidanceId, contentVersion, category, replayed: true };
      const rows = await sql`
        insert into lifemate.women_companion_guidance_history
          (relationship_id,patient_person_id,caregiver_person_id,guidance_id,content_version,category,shown_at_utc)
        values (${relationshipId}::uuid,${patientPersonId}::uuid,${caregiverPersonId}::uuid,
          ${guidanceId},${contentVersion},${category},now()) returning id,shown_at_utc
      `;
      await insertAudit(sql, caregiverAppUserId, "women_calendar.guidance_impression_recorded", "women_companion_guidance", String(rows[0].id));
      return { id: String(rows[0].id), guidanceId, contentVersion, category, shownAtUtc: iso(rows[0].shown_at_utc), replayed: false };
    }
    ...
    if (!privacy.viewSharedWellbeing) throw accessDenied();
    const profiles = await sql`
    ...
    const rows = await sql`
      ...
      values (${id}::uuid,${caregiverAppUserId}::uuid,${relationshipId}::uuid,${actionType},${now},${now},${patientPersonId}::uuid)
  ...
  return { ...base, getCareSummary, recordCareSupportAction };
}
...
function companionPrivacy(row: Row): CompanionPrivacy { return {
...
function accessDenied(): ApiError { return new ApiError(403, "women_calendar_access_denied", "Women calendar access is not active."); }
function mapEpisodeCaregiver(row: Row): Record<string, unknown> { return { id: row.id, startedOn: dateString(row.started_on), endedOn: row.ended_on == null ? null : dateString(row.ended_on), version: row.version }; }
function mapDailyLogCompanion(row: Row): Record<string, unknown> { return { loggedOn: dateString(row.logged_on), mood: String(row.mood).toLowerCase(), energyLevel: row.energy_level, version: row.version, updatedAtUtc: iso(row.updated_at_utc) }; }
async function insertAudit(connection: any, actorAppUserId: string, action: string, resourceType: string, resourceId: string): Promise<void> {
  await connection`insert into lifemate.audit_logs (id,actor_user_id,action,resource_type,resource_id,metadata_json,created_at_utc)
    values (${crypto.randomUUID()}::uuid,${actorAppUserId}::uuid,${action},${resourceType},${resourceId}::uuid,null,now())`;
}
