...
async function claimMedication(
  ...
  return await sql`
    with candidates as materialized (
      ...
      select ${caregiverPersonId}::uuid,c.relationship_id::uuid,c.source_key,
...
async function claimCareEvents(
  ...
  return await sql`
    with candidates as materialized (
      select
        'care-event:' || event.id::text || ':' || event.completed_at_utc::text as source_key,
...
async function medicationHistory(
  ...
  return await sql`
    select receipt.source_key,e.id::text as source_event_id,
      'medication'::text as source_kind,r.id::text as relationship_id,
      r.patient_user_id::text as patient_user_id,
      profile.display_name as patient_display_name,m.name as item_name,p.dose_text,
      e.previous_status,e.resulting_status,e.event_type,e.occurred_at_utc,e.recorded_at_utc,
      r.caregiver_lock_screen_detail
    ...
    join lifemate.medications m
      on m.id = p.medication_id and m.owner_person_id = o.patient_person_id
    join lifemate.care_relationships r
      on r.id = ${relationshipId}::uuid and r.id = receipt.relationship_id
     and r.patient_person_id = o.patient_person_id
     and r.caregiver_person_id = ${caregiverPersonId}::uuid
     and r.status = 'Active'
     and r.patient_consented_at_utc is not null
    ...
    where receipt.caregiver_person_id = ${caregiverPersonId}::uuid
      and o.status = 'Taken' and e.resulting_status = 'Taken'
...
async function careEventHistory(
  ...
  return await sql`
    select receipt.source_key,event.id::text as source_event_id,
      lower(event.event_type)::text as source_kind,r.id::text as relationship_id,
      r.patient_user_id::text as patient_user_id,
      profile.display_name as patient_display_name,
      coalesce(event.medication_name,event.title) as item_name,
      coalesce(event.dose_text,'') as dose_text,
      ...
      'completed'::text as resulting_status,'completed'::text as event_type,
      event.completed_at_utc as occurred_at_utc,event.updated_at_utc as recorded_at_utc,
      r.caregiver_lock_screen_detail
    from lifemate.caregiver_completion_notification_receipts receipt
    ...
    join core.person_profiles profile on profile.person_id = event.patient_person_id
    where receipt.caregiver_person_id = ${caregiverPersonId}::uuid
      and event.status = 'Completed' and event.completed_at_utc is not null
    order by receipt.claimed_at_utc desc
    limit 50
  `;
}
...
function mapNotification(row: Row): Record<string, unknown> {
  const kind = String(row.source_kind ?? "medication").toLowerCase();
  return {
    ...
    medicationName: row.item_name ?? "Treatment",
