...
export function createPersonMedicationStore(databaseUrl: string) {
  ...
  async function createMedication(appUserId:string, body:Record<string,unknown>):Promise<Record<string,unknown>> {
    ...
    return await sql.begin(async (tx:any) => {
      ...
      const countRows=await tx`select count(*)::integer as count from lifemate.medications where owner_person_id=${personId}::uuid`;
      try {
        await tx`select commerce.assert_free_quota(${appUserId}::uuid,'free.medications.max',${Number(countRows[0]?.count ?? 0)}::integer)`;
      } catch (error) {
        if (String((error as Record<string,unknown>)?.message ?? '').includes('premium_required_quota_reached')) {
          throw new ApiError(403,'premium_required_quota_reached','Premium is required to add another active medication.');
        }
        throw error;
      }
      const rows=await tx`
      ...
      `;
      await insertAudit(tx,appUserId,"medication.created","medication",String(rows[0].id));
      return mapMedication(rows[0]);
    });
  }

  async function listMedications(appUserId:string):Promise<Record<string,unknown>[]> {
    const personId=await requireSelfPerson(sql,appUserId);
    const rows=await sql`select * from lifemate.medications where owner_person_id=${personId}::uuid order by name,id limit 100`;
