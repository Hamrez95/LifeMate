...
type CircleCommand =
  ...
  | { action: "respond_invite"; invitationId: string; response: "accept" | "decline" }
  | { action: "set_sharing"; circleId: string; mode: string; includePeriodWindow?: boolean; includePhaseContext?: boolean; includeWellbeingContext?: boolean; version?: number }
...
export function createWomenCircleStore(databaseUrl: string) {
  ...
  async function execute(
    ...
    if (command.action === "create") {
      return await sql.begin(async (tx: any) => {
        ...
        const rows = await tx`
          ...
          values(${id}::uuid,${actorPersonId}::uuid,'women_health_planning',${command.name},${command.iconKey ?? null},'active',1,now(),now())
          returning *
        `;
        await tx`
    ...
    if (command.action === "invite") {
      ...
      if (inviteePersonId === actorPersonId) throw invalid("cannot_invite_self");
      return await sql.begin(async (tx: any) => {
        const circle = await requireOwnerCircle(tx, command.circleId, actorPersonId);
        const existingMember = await tx`
          ...
          limit 1
        `;
        if (existingMember[0]) throw new ApiError(409,"circle_member_exists","Person is already an active Circle member.");
        ...
        `;
        if (existingInvite[0]) {
          return { invitationId: String(existingInvite[0].id), idempotentReplay: true, expiresAtUtc: iso(existingInvite[0].expires_at_utc) };
        }
        ...
        `;
        await audit(tx,String(circle.id),actorPersonId,"circle.invitation_created",inviteePersonId);
        return { invitationId: String(rows[0].id), expiresAtUtc: iso(rows[0].expires_at_utc) };
      });
    }
    ...
    if (command.action === "respond_invite") {
      return await sql.begin(async (tx: any) => {
        ...
        if (invite.status === "accepted" && command.response === "accept") {
          return { accepted: true, idempotentReplay: true, circleId: String(invite.circle_id) };
        }
        if (invite.status !== "pending" || new Date(invite.expires_at_utc).getTime() <= Date.now()) {
          throw new ApiError(409,"circle_invitation_not_pending","Circle invitation is no longer pending.");
        }
        if (command.response === "decline") {
          ...
          await audit(tx,String(invite.circle_id),actorPersonId,"circle.invitation_declined",actorPersonId);
        ...
        await tx`
          ...
          on conflict(circle_id,person_id) do nothing
        `;
        await audit(tx,String(invite.circle_id),actorPersonId,"circle.invitation_accepted",actorPersonId);
    ...
    if (command.action === "set_sharing") {
      return await sql.begin(async (tx: any) => {
        ...
        if (command.mode !== "none" && policy.mode === "none") throw invalid("invalid_circle_sharing_mode");
        ...
        if (command.version != null && command.version !== version) {
          throw new ApiError(409,"stale_circle_sharing_policy","Circle sharing policy changed. Refresh and try again.");
        }
        const rows = await tx`
          insert into network.circle_member_sharing_policies(
            ...
            ${command.circleId}::uuid,${actorPersonId}::uuid,${policy.mode},${policy.includePeriodWindow},${policy.includePhaseContext},${policy.includeWellbeingContext},1,now(),now(),${policy.mode === "none" ? new Date() : null}
          )
          ...
          returning *
        `;
        await audit(tx,command.circleId,actorPersonId,"circle.sharing_updated",actorPersonId);
    ...
    if (command.action === "leave") {
      return await sql.begin(async (tx: any) => {
        const circle = await tx`select * from network.circles where id=${command.circleId}::uuid and status='active' limit 1`;
        if (!circle[0]) throw notFound();
        if (String(circle[0].owner_person_id) === actorPersonId) {
          throw new ApiError(409,"circle_owner_cannot_leave","Circle owner must close the Circle instead.");
        }
        const rows = await tx`
        ...
        await audit(tx,command.circleId,actorPersonId,"circle.member_left",actorPersonId);
        return { left: true };
      });
    }
    ...
    if (command.action === "remove_member") {
      return await sql.begin(async (tx: any) => {
        ...
        if (command.memberPersonId === actorPersonId) throw invalid("cannot_remove_circle_owner");
        const rows = await tx`
        ...
        await audit(tx,command.circleId,actorPersonId,"circle.member_removed",command.memberPersonId);
        return { removed: true };
      });
    }
    ...
    if (command.action === "close") {
      return await sql.begin(async (tx: any) => {
        ...
        await audit(tx,command.circleId,actorPersonId,"circle.closed",actorPersonId);
        return { closed: true };
      });
    }
...
async function mapCircle(connection: any, circle: Row, viewerPersonId: string): Promise<Record<string, unknown>> {
  ...
  `;
  const policyByPerson = new Map(policies.map((row: Row) => [String(row.person_id),row]));
  const contributions = [];
  for (const member of members) {
    ...
    contributions.push(deriveCirclePlanningContribution(policy,estimate,false));
  }
  ...
  return {
    ...
    ownSharing: ownPolicy ? mapSharing(ownPolicy) : { mode: "none",includePeriodWindow:false,includePhaseContext:false,includeWellbeingContext:false,version:0 },
    version: Number(circle.version),
  };
}

async function safeEstimate(connection: any, personId: string): Promise<any | null> {
  ...
  return calculateWomenCalendarEstimateFromEpisodes(
    dateString(row.last_period_start),Number(row.cycle_length),Number(row.period_length),
...
function normalizeCommand(raw: Record<string, unknown>): CircleCommand {
  ...
  if (action === "create") {
    ...
    if (name.length < 1 || name.length > 80) throw invalid("invalid_circle_name");
    const iconKey = text(raw.iconKey);
    if (iconKey != null && iconKey.length > 48) throw invalid("invalid_circle_icon");
    return { action:"create",name,iconKey };
  }
  if (action === "invite") return { action:"invite",circleId:uuid(raw.circleId,"circleId"),inviteeAppUserId:uuid(raw.inviteeAppUserId,"inviteeAppUserId") };
  if (action === "respond_invite") {
    const response = String(raw.response ?? "").toLowerCase();
    if (response !== "accept" && response !== "decline") throw invalid("invalid_circle_invitation_response");
    return { action:"respond_invite",invitationId:uuid(raw.invitationId,"invitationId"),response };
  }
  if (action === "set_sharing") {
    const version = raw.version == null ? undefined : Number(raw.version);
    if (version != null && (!Number.isInteger(version) || version < 0)) throw invalid("invalid_circle_version");
    return {
      action:"set_sharing",circleId:uuid(raw.circleId,"circleId"),mode:String(raw.mode ?? "none").toLowerCase(),
      includePeriodWindow:raw.includePeriodWindow===true,includePhaseContext:raw.includePhaseContext===true,
      includeWellbeingContext:raw.includeWellbeingContext===true,version,
    };
  }
  if (action === "leave") return { action:"leave",circleId:uuid(raw.circleId,"circleId") };
  if (action === "remove_member") return { action:"remove_member",circleId:uuid(raw.circleId,"circleId"),memberPersonId:uuid(raw.memberPersonId,"memberPersonId") };
  if (action === "close") return { action:"close",circleId:uuid(raw.circleId,"circleId") };
  throw invalid("unsupported_circle_command");
}

async function requireOwnerCircle(connection:any,circleId:string,personId:string):Promise<Row>{
  const rows=await connection`select * from network.circles where id=${circleId}::uuid and owner_person_id=${personId}::uuid and status='active' for update`;
  if(!rows[0]) throw notFound();
  return rows[0];
}

async function requireActiveMember(connection:any,circleId:string,personId:string):Promise<void>{
  const rows=await connection`select 1 from network.circle_members m join network.circles c on c.id=m.circle_id where m.circle_id=${circleId}::uuid and m.person_id=${personId}::uuid and m.membership_status='active' and c.status='active' limit 1`;
  if(!rows[0]) throw notFound();
}
...
async function selfPersonId(connection:any,appUserId:string):Promise<string>{
  ...
  if(typeof value!=="string"||value.length===0) throw new ApiError(409,"identity_person_mapping_missing","The LifeMate person mapping is unavailable.");
  return value;
}

async function audit(connection:any,circleId:string,actorPersonId:string,action:string,subjectPersonId:string):Promise<void>{
...
function uuid(value:unknown,field:string):string{
  ...
  if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(textValue)) throw invalid(`invalid_${field}`);
  return textValue;
}
function text(value:unknown):string|null{const v=value==null?"":String(value).trim();return v.length===0?null:v;}
function iso(value:unknown):string|null{if(value==null)return null;const d=value instanceof Date?value:new Date(String(value));return Number.isFinite(d.getTime())?d.toISOString():null;}
function dateString(value:unknown):string{if(value instanceof Date)return value.toISOString().slice(0,10);return String(value).slice(0,10);}
function invalid(code:string):ApiError{return new ApiError(400,code,"Invalid Women Health Circle request.");}
function notFound():ApiError{return new ApiError(404,"women_circle_not_found","Women Health Circle was not found or is not accessible.");}
