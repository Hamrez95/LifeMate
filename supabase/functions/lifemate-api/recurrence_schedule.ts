...
function integer(value: unknown, field: string, min: number, max: number): number {
  ...
  if (!Number.isInteger(parsed) || parsed < min || parsed > max) {
    throw new ApiError(400, `invalid_${field}`, `${field} is outside the supported range.`);
  }
  return parsed;
}
...
export function normalizeRecurrenceRule(value: unknown): RecurrenceRule | null {
  ...
  const weekdays = Array.isArray(record.weekdays)
    ? [...new Set(record.weekdays.map((day) => integer(day, "recurrence_weekday", 1, 7)))].sort()
    : [];
  if (unit !== "week" && weekdays.length > 0) {
    throw new ApiError(400, "invalid_recurrence_weekdays", "Weekdays are only valid for weekly recurrence.");
  }
  ...
  return {
    ...
    interval: integer(record.interval ?? 1, "recurrence_interval", 1, unit === "hour" ? 8760 : 365),
    weekdays,
    endAt,
...
export function expandLocalRecurrence(
  ...
  const explicitEnd = rule.endAt == null ? null : parseLocalTimestamp(rule.endAt);
  const to = explicitEnd != null && explicitEnd < requestedTo ? explicitEnd : requestedTo;
  ...
  const accept = (cursor: Date): boolean => {
    ...
    if (cursor >= from && cursor <= to) {
      ...
      if (result.length > maxReturned) {
        throw new ApiError(400, "recurrence_window_too_dense", "Recurrence produces too many occurrences in this window.");
      }
    }
  ...
  if (rule.unit === "hour" || rule.unit === "day") {
    const milliseconds = rule.interval * (rule.unit === "hour" ? 3600000 : 86400000);
    for (let cursor = start, guard = 0; cursor <= to && guard < 100000; guard++) {
      if (!accept(cursor)) break;
      cursor = new Date(cursor.getTime() + milliseconds);
    }
  } else if (rule.unit === "week") {
    const allowed = new Set(rule.weekdays.length > 0 ? rule.weekdays : [isoWeekday(start)]);
    for (let cursor = start, guard = 0; cursor <= to && guard < 100000; guard++) {
    ...
    for (let occurrence = 0; occurrence < max; occurrence++) {
      const cursor = addMonthsClamped(start, occurrence * rule.interval * monthFactor);
      if (cursor > to) break;
      if (!accept(cursor)) break;
    }
  }
...
export function parseLocalTimestamp(value: string): Date {
  ...
  if (!match) throw new ApiError(400, "invalid_local_timestamp", "Invalid local date/time.");
  ...
  if (
    ...
    throw new ApiError(400, "invalid_local_timestamp", "Invalid local date/time.");
  }
  return date;
}
...
function addDaysPreservingClock(value: Date, days: number): Date {
  return new Date(Date.UTC(
    value.getUTCFullYear(), value.getUTCMonth(), value.getUTCDate() + days,
...
function addMonthsClamped(value: Date, months: number): Date {
  ...
  return new Date(Date.UTC(
    first.getUTCFullYear(), first.getUTCMonth(), Math.min(value.getUTCDate(), lastDay),
    value.getUTCHours(), value.getUTCMinutes(), value.getUTCSeconds(),
  ));
}

function calendarDayDistance(start: Date, cursor: Date): number {
  const left = Date.UTC(start.getUTCFullYear(), start.getUTCMonth(), start.getUTCDate());
  const right = Date.UTC(cursor.getUTCFullYear(), cursor.getUTCMonth(), cursor.getUTCDate());
