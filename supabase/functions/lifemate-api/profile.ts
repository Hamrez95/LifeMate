...
export function createProfileStore(
  ...
  async function updateProfile(
    ...
    return {
      ...updatedProfile,
      // The shared runtime database client intentionally has max=1. Read the
      // preference projection only after the write transaction has committed so
      // this response enrichment never tries to acquire the same single pooled
      // connection while it is held by an open transaction.
      privacyPreferences: await privacyPreferences.preferences(userId),
    };
  }
