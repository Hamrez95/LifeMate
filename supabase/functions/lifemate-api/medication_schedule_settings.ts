...
import { ApiError, normalizeOptional, requiredPositiveInt } from "./validation.ts";
...
function requiredBoolean(value: unknown, field: string): boolean {
  if (value !== true && value !== false) {
    throw new ApiError(400, `invalid_${field}`, `${field} must be a boolean.`);
  }
  return value;
}
...
function optionalLocalTime(value: unknown, field: string): string | null {
  ...
  if (!/^(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d)?$/.test(normalized)) {
    throw new ApiError(400, `invalid_${field}`, `${field} must be a local time.`);
  }
...
function optionalTimingNote(value: unknown): string | null {
  ...
  if (normalized.length > maxTimingNoteLength) {
    throw new ApiError(400, "invalid_timing_note", "timingNote is too long.");
  }
  return normalized;
}
...
export function createMedicationScheduleSettingsStore(databaseUrl: string) {
  ...
  async function requireSelfPerson(appUserId: string, connection: any = sql) {
    ...
    if (typeof personId !== "string" || personId.length === 0) {
      throw new ApiError(409, "identity_person_mapping_missing", "Person mapping is unavailable.");
    }
    return personId;
  }
  ...
  async function requireIanaTimeZone(value: unknown): Promise<string> {
    ...
    const rows = await sql`select exists(select 1 from pg_timezone_names where name=${zone}) as valid`;
    if (rows[0]?.valid !== true) {
      throw new ApiError(400, "invalid_time_zone", "timeZone must be a valid IANA time zone.");
    }
    return zone;
  }

  async function getPreferences(appUserId: string): Promise<Record<string, unknown>> {
    ...
    const rows = await sql`
      ...
      limit 1
    `;
    if (!rows[0]) throw new ApiError(404, "profile_missing", "Profile was not found.");
    const row = rows[0];
    return {
      ...
      updatedAtUtc: row.updated_at_utc == null ? null : new Date(row.updated_at_utc).toISOString(),
    };
  }
  ...
  async function updatePreferences(
    ...
    const enabled = requiredBoolean(body.sleepWindowEnabled, "sleep_window_enabled");
    const start = optionalLocalTime(body.sleepStartLocalTime, "sleep_start_local_time");
    const end = optionalLocalTime(body.sleepEndLocalTime, "sleep_end_local_time");
    ...
    const changed = await sql.begin(async (tx: any) => {
      if (expectedVersion === 0) {
        return await tx`
          insert into lifemate.medication_schedule_preferences
            (owner_person_id,time_zone,sleep_window_enabled,sleep_start_local_time,
             sleep_end_local_time,version,created_at_utc,updated_at_utc)
          values
            (${personId}::uuid,${timeZone},${enabled},${enabled ? start : null}::time,
             ${enabled ? end : null}::time,1,now(),now())
      ...
      return await tx`
        ...
        set time_zone=${timeZone}, sleep_window_enabled=${enabled},
            sleep_start_local_time=${enabled ? start : null}::time,
            sleep_end_local_time=${enabled ? end : null}::time,
            version=version+1, updated_at_utc=now()
        where owner_person_id=${personId}::uuid and version=${expectedVersion}
        returning version
      `;
    });
    if (!changed[0]) {
      throw new ApiError(409, "stale_schedule_preferences", "Schedule preferences changed. Refresh and try again.");
    }
  ...
  async function getPlanTiming(
    ...
    const rows = await sql`
      select p.id, p.version as plan_version,
             ...
             c.timing_note,c.version,c.updated_at_utc
      from lifemate.treatment_plans p
      left join lifemate.treatment_plan_timing_constraints c
        on c.treatment_plan_id=p.id
      where p.id=${treatmentPlanId}::uuid and p.patient_person_id=${personId}::uuid
      limit 1
    `;
    if (!rows[0]) throw new ApiError(404, "treatment_plan_missing", "Treatment plan was not found.");
    const row = rows[0];
    return {
  ...
  async function updatePlanTiming(
    ...
    const expectedVersion = requiredVersion(body.version);
    const expectedPlanVersion = requiredPositiveInt(body.treatmentPlanVersion, "treatmentPlanVersion");
    const nearby = requiredBoolean(body.nearbyGroupingEnabled, "nearby_grouping_enabled");
    const locked = requiredBoolean(body.timingLocked, "timing_locked");
    const before = requiredSpacing(body.manualSpacingBeforeMinutes, "manual_spacing_before_minutes");
    const after = requiredSpacing(body.manualSpacingAfterMinutes, "manual_spacing_after_minutes");
    const note = optionalTimingNote(body.timingNote);
    const personId = await requireSelfPerson(appUserId);

    const changed = await sql.begin(async (tx: any) => {
      const plan = await tx`
        select version from lifemate.treatment_plans
        where id=${treatmentPlanId}::uuid and patient_person_id=${personId}::uuid
        for update
      `;
      if (!plan[0]) throw new ApiError(404, "treatment_plan_missing", "Treatment plan was not found.");
      if (Number(plan[0].version) !== expectedPlanVersion) {
        throw new ApiError(409, "stale_treatment_plan", "Treatment plan changed. Refresh and try again.");
      }
      if (expectedVersion === 0) {
        return await tx`
          insert into lifemate.treatment_plan_timing_constraints
            (treatment_plan_id,owner_person_id,nearby_grouping_enabled,timing_locked,
             manual_spacing_before_minutes,manual_spacing_after_minutes,timing_note,
      ...
      return await tx`
        ...
        set nearby_grouping_enabled=${nearby}, timing_locked=${locked},
            manual_spacing_before_minutes=${before}, manual_spacing_after_minutes=${after},
            timing_note=${note}, version=version+1, updated_at_utc=now()
        where treatment_plan_id=${treatmentPlanId}::uuid
          and owner_person_id=${personId}::uuid and version=${expectedVersion}
        returning version
      `;
    });
    if (!changed[0]) {
      throw new ApiError(409, "stale_timing_constraints", "Timing constraints changed. Refresh and try again.");
    }
    return await getPlanTiming(appUserId, treatmentPlanId);
  }

  return { getPreferences, updatePreferences, getPlanTiming, updatePlanTiming };
}
