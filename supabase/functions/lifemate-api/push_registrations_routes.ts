...
const registrationPath = /^\/api\/v1\/messaging\/push-registrations\/([0-9a-f-]{36})$/i;
...
export function createPushRegistrationRouteHandler(databaseUrl: string) {
  ...
  return async function handlePushRegistrationRoute(input: {
    ...
    if (input.request.method === "POST" && input.path === "/api/v1/messaging/push-registrations") {
      const result = await store.upsert(input.appUserId, await readJsonObject(input.request));
    ...
    if (input.request.method === "DELETE" && match) {
      const result = await store.revoke(input.appUserId, match[1].toLowerCase());
      return json(result, Number(result.httpStatus ?? 200));
    }
