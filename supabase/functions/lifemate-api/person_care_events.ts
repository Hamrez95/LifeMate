  ...
  async function createCareEvent(
    ...
    return await sql.begin(async (tx: any) => {
      ...
      if (existingRows[0]) {
        ...
        return mapCareEvent(existingRows[0], patientAppUserId);
      }
  ...
  async function selectCareEvents(
    ...
    for (const row of rows) {
      ...
      for (const occurrenceDate of dates) {
        expanded.push(
          mapCareEvent(row, patientAppUserId, occurrenceDate),
        );
      }
    }
...
function mapCareEvent(
  ...
  return {
    ...
    // Public compatibility field derives from request context rather than the
    // nullable legacy ownership column.
...
function localDateTimeToUtc(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const parts = Object.fromEntries(
      formatter.formatToParts(new Date(guess)).map((part) => [
        part.type,
        part.value,
      ]),
    );
