...
export function validateSupportAttachment(
  ...
  if (bytes.length < 1 || bytes.length > supportAttachmentMaximumBytes) {
    throw new ApiError(413, "support_attachment_too_large", "Attachment must be between 1 byte and 10 MB.");
  }
  ...
  if (type === "image/jpeg" && jpeg) return { contentType: "image/jpeg", extension: "jpg" };
  if (type === "image/png" && png) return { contentType: "image/png", extension: "png" };
  if (type === "image/webp" && webp) return { contentType: "image/webp", extension: "webp" };
  if (type === "application/pdf" && pdf) return { contentType: "application/pdf", extension: "pdf" };
...
export function createSupportAttachmentRuntime(supabaseUrl: string, serviceRoleKey: string) {
  ...
  const scannerToken = (Deno.env.get("LIFEMATE_SUPPORT_ATTACHMENT_SCAN_TOKEN") ?? "").trim();
  ...
  async function ensureBucket() {
    bucketPromise ??= (async () => {
      const current = await fetch(`${storageRoot}/bucket/${supportAttachmentBucket}`, { headers: storageHeaders() });
      if (current.ok) return;
      if (current.status !== 400 && current.status !== 404) throw storageUnavailable();
  ...
  async function upload(accountId: string, ticketId: string, messageId: string, bytes: Uint8Array, rawContentType: string) {
    const validated = validateSupportAttachment(bytes, rawContentType);
    await ensureBucket();
    const objectPath = `${accountId}/${ticketId}/${messageId}/${crypto.randomUUID()}.${validated.extension}`;
    assertSafePath(objectPath);
    const response = await fetch(`${storageRoot}/object/${supportAttachmentBucket}/${encodePath(objectPath)}`, {
      method: "POST",
      headers: { ...storageHeaders(validated.contentType), "x-upsert": "false", "cache-control": "no-store" },
      body: bytes,
    });
  ...
  async function scan(bytes: Uint8Array, contentType: string, fileName: string): Promise<SupportAttachmentScanResult> {
    if (!scannerUrl || !scannerToken) return { status: "ScanError", reasonCode: "scanner_unconfigured" };
    try {
      ...
      if (!response.ok) return { status: "ScanError", reasonCode: `scanner_http_${response.status}` };
      const payload = await response.json().catch(() => null) as Record<string, unknown> | null;
      if (payload?.clean === true) return { status: "Available", reasonCode: null };
      if (payload?.clean === false) return { status: "Rejected", reasonCode: "malware_detected" };
  ...
  async function signedDownload(objectPath: string): Promise<string> {
    assertSafePath(objectPath);
    await ensureBucket();
    const response = await fetch(`${storageRoot}/object/sign/${supportAttachmentBucket}/${encodePath(objectPath)}`, {
      method: "POST",
      headers: storageHeaders("application/json"),
    ...
    if (typeof signed !== "string" || signed.length === 0) throw storageUnavailable();
  ...
  async function remove(objectPath: string) {
    assertSafePath(objectPath);
    await ensureBucket();
    const response = await fetch(`${storageRoot}/object/${supportAttachmentBucket}`, {
      method: "DELETE",
      headers: storageHeaders("application/json"),
...
function assertSafePath(value: string) {
  if (!/^[0-9a-f-]{36}\/[0-9a-f-]{36}\/[0-9a-f-]{36}\/[0-9a-f-]{36}\.(jpg|png|webp|pdf|txt)$/i.test(value) || value.includes("..")) {
    throw new ApiError(400, "support_attachment_path_invalid", "Attachment path is invalid.");
  }
}

function storageUnavailable() {
  return new ApiError(503, "support_attachment_storage_unavailable", "Attachment storage is temporarily unavailable.");
}
