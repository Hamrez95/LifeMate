...
async function route(
  ...
  if (healthObservationMatch && request.method === "GET") {
    return json(
      await healthObservations.getOwnerObservation(
        identity.appUserId,
        healthObservationMatch[1],
      ),
    );
  }
  if (healthObservationMatch && request.method === "PATCH") {
    enforceRateLimit(`health-write:${identity.appUserId}`, 60, 60 * 60_000);
    return json(
      await healthObservations.updateOwnerObservation(
        identity.appUserId,
        healthObservationMatch[1],
        await readJsonObject(request),
      ),
  ...
  if (healthObservationMatch && request.method === "DELETE") {
    enforceRateLimit(`health-write:${identity.appUserId}`, 60, 60 * 60_000);
  ...
  const doseReportMatch = path.match(
    /^\/api\/v1\/dose-occurrences\/([0-9a-f-]{36})\/report$/i,
  );
  if (request.method === "POST" && doseReportMatch) {
    return json(
      await db.reportDose(
        identity.appUserId,
        doseReportMatch[1],
  ...
  const careEventOwnerMatch = path.match(
    /^\/api\/v1\/care-events\/([0-9a-f-]{36})$/i,
  );
  if (careEventOwnerMatch && request.method === "GET") {
    return json(
      await careEvents.getCareEvent(identity.appUserId, careEventOwnerMatch[1]),
    );
  }
  if (careEventOwnerMatch && request.method === "PATCH") {
    return json(
      await careEvents.updateCareEvent(
        identity.appUserId,
        careEventOwnerMatch[1],
        await readJsonObject(request),
      ),
    );
  }
  if (careEventOwnerMatch && request.method === "DELETE") {
    await careEvents.deleteCareEvent(identity.appUserId, careEventOwnerMatch[1]);
    return json({ deleted: true });
  }

  if (request.method === "GET" && path === "/api/v1/care/relationships") {
    return json(await presentRelationships(await db.listRelationships(identity.appUserId)));
  }
  ...
  const relationshipPermissionsMatch = path.match(
    /^\/api\/v1\/care\/relationships\/([0-9a-f-]{36})\/permissions$/i,
  );
  if (relationshipPermissionsMatch && request.method === "PATCH") {
    return json(
      await db.updateRelationshipPermissions(
        identity.appUserId,
        relationshipPermissionsMatch[1],
        await readJsonObject(request),
      ),
    );
  }
  ...
  const womenPrivacyMatch = path.match(
    /^\/api\/v1\/women-calendar\/companion-privacy\/([0-9a-f-]{36})$/i,
  );
  if (request.method === "PUT" && womenPrivacyMatch) {
    requireWomenCalendarPilot();
...
async function presentRelationships(
  ...
  return await Promise.all(relationships.map(async (relationship) => {
    ...
    const patientProfile = await presentProfile(patientUserId).catch(() => null);
    const caregiverProfile = await presentProfile(caregiverUserId).catch(() => null);
    return {
      ...relationship,
      patientDisplayName: patientProfile?.displayName ?? null,
      patientProfilePhotoUrl: patientProfile?.profilePhotoUrl ?? null,
      patientAvatarKey: patientProfile?.avatarKey ?? null,
      patientPhoneNumber: patientProfile?.phoneNumber ?? null,
      caregiverDisplayName: caregiverProfile?.displayName ?? null,
...
function requireWomenCalendarPilot(): void {
  if (!womenCalendarPilotEnabled) {
    throw new ApiError(
      ...
      "Women Calendar pilot is not enabled.",
    );
  }
}
...
function isPostgresConflict(error: unknown): boolean {
  ...
  return code === "23505" || code === "23503" || code === "40001";
}
