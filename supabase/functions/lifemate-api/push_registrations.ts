...
function required(value: unknown, field: string, max: number): string {
  if (typeof value !== "string") throw new ApiError(400, "push_registration_invalid", `${field} is invalid.`);
  const next = value.trim();
  if (!next || new TextEncoder().encode(next).byteLength > max) {
    throw new ApiError(400, "push_registration_invalid", `${field} is invalid.`);
  }
  return next;
}

export function parsePushRegistration(body: Record<string, unknown>) {
  const productCode = required(body.productCode, "productCode", 64).toLowerCase();
  ...
  if (!productPattern.test(productCode) || !providerPattern.test(provider) ||
      !["Android", "iOS", "Web"].includes(platform) || token.length < 20) {
    throw new ApiError(400, "push_registration_invalid", "Push registration is invalid.");
  }
...
function result(result: Record<string, unknown>): Record<string, unknown> {
  ...
  if (!Number.isInteger(status) || status < 100 || status > 599) {
    throw new ApiError(503, "push_registration_unavailable", "Push registration workflow returned an invalid result.");
  }
  if (status >= 400) {
    throw new ApiError(status, String(result.code ?? "push_registration_unavailable"),
      typeof result.message === "string" ? result.message : "Push registration could not be completed.");
  }
  return result;
}
  ...
  return {
    ...
    async revoke(appUserId: string, registrationId: string) {
      if (!uuidPattern.test(registrationId)) {
        throw new ApiError(400, "push_registration_invalid", "registrationId is invalid.");
      }
      const rows = await sql`
