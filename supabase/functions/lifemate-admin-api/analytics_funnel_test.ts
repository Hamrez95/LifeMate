...
Deno.test("activation funnel stages are ordered and privacy-bounded", () => {
  const stages = KPI_DEFINITIONS.filter((definition) => definition.funnel?.id === "activation");

  assert(stages.length === 2, "activation funnel must expose exactly two truthful stages");
  assert(stages[0]?.name === "activation_enrolled_accounts", "stage 1 must be enrollment cohort");
  assert(stages[0]?.funnel?.stageOrder === 1, "stage 1 order must be stable");
  assert(stages[0]?.funnel?.previousStage === null, "stage 1 has no previous stage");
  assert(stages[1]?.name === "activation_observed_accounts", "stage 2 must be observed activation");
  ...
  assert(
    stages.every(
      (stage) => stage.funnel?.privacyThreshold === ACTIVATION_FUNNEL_PRIVACY_THRESHOLD,
    ),
...
Deno.test("activation rate is computed by Core from the same funnel cohort", () => {
  const rate = KPI_DEFINITIONS.find((definition) => definition.name === "activation_observed_rate");
