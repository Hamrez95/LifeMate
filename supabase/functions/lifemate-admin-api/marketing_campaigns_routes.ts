...
function mutationStatus(result: Record<string, unknown>): number {
  ...
  if (!Number.isInteger(status) || status < 100 || status > 599) {
    throw new ApiError(503, "marketing_workflow_unavailable", "Marketing workflow returned an invalid status.");
  }
  return status;
}

function mutationErrorMessage(result: Record<string, unknown>, fallback: string): string {
...
export function createMarketingCampaignRouteHandler(databaseUrl: string) {
  ...
  return async function handleMarketingCampaignRoute(context: MarketingCampaignRouteContext): Promise<Response | null> {
    ...
    const campaignOrchestratorResponse = await campaignOrchestratorRouteHandler(context);
    ...
    if (request.method === "GET" && path === "/api/v1/analytics/growth") {
      ...
      return json(await growthStore.read(parseGrowthAnalyticsQuery(new URL(request.url))), 200, origin);
    }
    if (request.method === "GET" && path === "/api/v1/commerce/revenue") {
      requirePermission(admin, "commerce.read");
      return json(await revenueStore.read(parseCommerceRevenueQuery(new URL(request.url))), 200, origin);
    }
    if (request.method === "GET" && path === "/api/v1/marketing/channels") {
      requirePermission(admin, "marketing.read");
      return json({ items: await channelStore.list(), freshness: { status: "fresh", asOfUtc: new Date().toISOString(), source: "admin.marketing_channel_connections_v1" } }, 200, origin);
    }
    if (request.method === "GET" && path === "/api/v1/marketing/attribution") {
      requirePermission(admin, "marketing.read");
      return json(await attributionStore.read(parseMarketingAttributionQuery(new URL(request.url))), 200, origin);
    }
    ...
    if (request.method === "POST" && channelProvider) {
      ...
      const requestHash = await hashMarketingChannelStatusRequest(channelProvider, payload);
      const result = await channelStore.setStatus({ actorAccountId: accountId, providerCode: channelProvider, payload, correlationId, idempotencyKey, requestHash });
      const status = mutationStatus(result);
      if (status >= 400) throw new ApiError(status, String(result.code), mutationErrorMessage(result, "Channel operator status change was not completed."));
      return json({ providerCode: String(result.providerCode), operatorStatus: String(result.operatorStatus), setupStatus: String(result.setupStatus), replayed: Boolean(result.replayed), providerConnectivity: "NotVerified" }, status, origin);
    }
    if (request.method === "GET" && path === "/api/v1/marketing/campaigns") {
      requirePermission(admin, "marketing.read");
      return json(await listMarketingCampaigns(campaignStore, parseMarketingCampaignQuery(new URL(request.url))), 200, origin);
    }
    if (request.method === "POST" && path === "/api/v1/marketing/campaigns") {
      ...
      const result = await campaignStore.create({ actorAccountId: accountId, payload, correlationId, idempotencyKey, requestHash });
      const status = mutationStatus(result);
      if (status >= 400) throw new ApiError(status, String(result.code), mutationErrorMessage(result, "Campaign creation was not completed."));
      return json({ campaignId: String(result.campaignId), status: String(result.status), replayed: Boolean(result.replayed) }, status, origin);
    }
    ...
    if (request.method === "POST" && statusCampaignId) {
      ...
      const requestHash = await hashMarketingCampaignStatusRequest(statusCampaignId, payload);
      const result = await campaignStore.setStatus({ actorAccountId: accountId, campaignId: statusCampaignId, payload, correlationId, idempotencyKey, requestHash });
      const status = mutationStatus(result);
      if (status >= 400) throw new ApiError(status, String(result.code), mutationErrorMessage(result, "Campaign status change was not completed."));
      return json({ campaignId: String(result.campaignId), previousStatus: String(result.previousStatus), status: String(result.status), noop: Boolean(result.noop), replayed: Boolean(result.replayed) }, status, origin);
    }
    ...
    if (request.method === "PUT" && campaignId) {
      ...
      const idempotencyKey = requireIdempotencyKey(request);
      const payload = await parseMarketingCampaignWritePayload(request);
      const requestHash = await hashUpdateMarketingCampaignRequest(campaignId, payload);
      const result = await campaignStore.update({ actorAccountId: accountId, campaignId, payload, correlationId, idempotencyKey, requestHash });
      const status = mutationStatus(result);
      if (status >= 400) throw new ApiError(status, String(result.code), mutationErrorMessage(result, "Campaign update was not completed."));
      return json({ campaignId: String(result.campaignId), previousStatus: String(result.previousStatus), status: String(result.status), replayed: Boolean(result.replayed) }, status, origin);
    }
    return null;
  };
}
