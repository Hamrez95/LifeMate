...
const actions = new Set(["Acknowledge", "Triage", "Resolve", "LinkSupport", "LinkProductIssue"]);

function optionalEnum(url: URL, key: string, allowed: Set<string>): string | null {
  const raw = url.searchParams.get(key)?.trim() ?? "";
  if (!raw) return null;
  if (!allowed.has(raw)) throw new ApiError(400, `feedback_${key}_invalid`, `${key} filter is invalid.`);
  return raw;
}
...
function optionalProduct(url: URL): string | null {
  ...
  if (!raw) return null;
  if (!productPattern.test(raw)) throw new ApiError(400, "feedback_product_invalid", "product filter is invalid.");
  return raw;
}
...
function optionalVersion(url: URL): string | null {
  ...
  if (!raw) return null;
  if (raw.length > 80) throw new ApiError(400, "feedback_app_version_invalid", "appVersion filter is invalid.");
  return raw;
}
...
function boundedInt(url: URL, key: string, fallback: number, min: number, max: number): number {
  ...
  if (!Number.isInteger(value) || value < min || value > max) {
    throw new ApiError(400, `feedback_${key}_invalid`, `${key} must be between ${min} and ${max}.`);
  }
  return value;
}
...
async function readJsonObject(request: Request): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    throw new ApiError(400, "feedback_payload_invalid", "Request body must be valid JSON.");
  }
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    throw new ApiError(400, "feedback_payload_invalid", "Request body must be a JSON object.");
  }
  return body as Record<string, unknown>;
}

function stringField(body: Record<string, unknown>, key: string, min: number, max: number): string {
...
function optionalUuid(body: Record<string, unknown>, key: string): string | null {
  ...
  if (typeof raw !== "string" || !uuidPattern.test(raw)) {
    throw new ApiError(400, `feedback_${key}_invalid`, `${key} must be a UUID.`);
  }
  return raw;
}
...
function optionalReference(body: Record<string, unknown>): string | null {
  ...
  if (raw === undefined || raw === null || raw === "") return null;
  if (typeof raw !== "string") throw new ApiError(400, "feedback_product_issue_ref_invalid", "productIssueRef is invalid.");
  const value = raw.trim();
  if (!value || value.length > 160 || /^(https?:\/\/|www\.)/i.test(value)) {
    throw new ApiError(400, "feedback_product_issue_ref_invalid", "productIssueRef must be an opaque internal reference.");
  }
  return value;
}
...
function resultStatus(result: Record<string, unknown>): number {
  ...
  if (!Number.isInteger(status) || status < 100 || status > 599) {
    throw new ApiError(503, "feedback_workflow_unavailable", "Feedback workflow returned an invalid status.");
  }
  if (status >= 400) {
    throw new ApiError(status, typeof result.code === "string" ? result.code : "feedback_workflow_failed", "Feedback workflow was not completed.");
  }
  return status;
}
...
export function createFeedbackAdminRouteHandler(databaseUrl: string) {
  ...
  return async function handleFeedbackAdminRoute(context: Context): Promise<Response | null> {
    ...
    if (request.method === "GET" && path === "/api/v1/feedback") {
      ...
      `;
      return json({ ...(rows[0]?.result as Record<string, unknown>), freshness: { status: "fresh", asOfUtc: new Date().toISOString() } }, 200, origin);
    }
    ...
    if (request.method === "POST" && actionMatch) {
      ...
      if (!uuidPattern.test(itemId)) throw new ApiError(400, "feedback_item_id_invalid", "Feedback item id is invalid.");
      const body = await readJsonObject(request);
      const expectedStatus = stringField(body, "expectedStatus", 1, 32);
      if (!statuses.has(expectedStatus)) throw new ApiError(400, "feedback_expected_status_invalid", "expectedStatus is invalid.");
      const action = stringField(body, "action", 1, 32);
      if (!actions.has(action)) throw new ApiError(400, "feedback_action_invalid", "action is invalid.");
      ...
      if (action === "LinkSupport" && supportTicketId === null) {
        throw new ApiError(400, "feedback_support_link_required", "supportTicketId is required for LinkSupport.");
      }
      if (action === "LinkProductIssue" && productIssueRef === null) {
        throw new ApiError(400, "feedback_product_issue_ref_required", "productIssueRef is required for LinkProductIssue.");
      }
      if (action !== "LinkSupport" && supportTicketId !== null) {
        throw new ApiError(400, "feedback_support_link_forbidden", "supportTicketId is only valid for LinkSupport.");
      }
      if (action !== "LinkProductIssue" && productIssueRef !== null) {
        throw new ApiError(400, "feedback_product_issue_ref_forbidden", "productIssueRef is only valid for LinkProductIssue.");
      }
      const idempotencyKey = requireIdempotencyKey(request);
      const requestHash = await sha256({ itemId, expectedStatus, action, reason, supportTicketId, productIssueRef });
      const rows = await sql`
