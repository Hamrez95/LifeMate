...
Deno.test("marketing attribution taxonomy never invents commercial facts", () => {
  assert(MARKETING_ATTRIBUTION_TAXONOMY.attributionState === "not_instrumented", "attribution must remain explicitly not instrumented");
  assert(
...
Deno.test("marketing attribution parser accepts only bounded canonical filters", () => {
  ...
  const query = parseMarketingAttributionQuery(new URL(`https://example.test/api/v1/marketing/attribution?from=2026-08-01&to=2026-08-26&product=wellmate&channel=instagram&campaignId=${campaignId}`));
...
Deno.test("marketing attribution parser rejects reversed date ranges", () => {
  let rejected = false;
  try {
    parseMarketingAttributionQuery(new URL("https://example.test/api/v1/marketing/attribution?from=2026-08-26&to=2026-08-01"));
  } catch {
    rejected = true;
  }
