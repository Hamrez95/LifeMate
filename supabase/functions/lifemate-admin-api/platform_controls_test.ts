...
Deno.test("platform control defaults fail closed", async () => {
  const result = await evaluatePlatformControl(definition, [], {
...
Deno.test("higher-priority product rule wins deterministically", async () => {
  ...
  const result = await evaluatePlatformControl(definition, rules, {
    subjectKey: "account-1",
...
Deno.test("percentage assignment is stable for the same opaque subject", async () => {
  ...
  const first = await evaluatePlatformControl(definition, rules, {
    subjectKey: "8e1d7d0c-opaque",
  });
  const second = await evaluatePlatformControl(definition, rules, {
    subjectKey: "8e1d7d0c-opaque",
  });
