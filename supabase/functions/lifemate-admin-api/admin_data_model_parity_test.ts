...
Deno.test("relationship overview keeps natural and care relationships semantically distinct", async () => {
  ...
  assertStringIncludes(relationships, "from admin.care_relationship_directory_v1");
...
Deno.test("care relationship compatibility view cannot fabricate a natural relationship type", async () => {
  const migration = await Deno.readTextFile(
    new URL("../../migrations/20260825151000_add_admin_care_relationship_directory.sql", import.meta.url),
  );

  assertStringIncludes(migration, "create or replace view admin.care_relationship_directory_v1");
  assertStringIncludes(migration, "from lifemate.care_relationships relationship");
