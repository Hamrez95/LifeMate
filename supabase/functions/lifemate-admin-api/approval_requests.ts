...
function objectBody(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new ApiError(400,"approval_payload_invalid","Approval payload must be an object.");
  }
...
function boundedReason(value: unknown): string {
  ...
  if (reason.length < 10 || reason.length > 1000) {
    throw new ApiError(400,"approval_reason_invalid","Reason must contain between 10 and 1000 characters.");
  }
  return reason;
}
...
export async function parseCreateApprovalRequest(request: Request): Promise<CreateApprovalRequest> {
...
export async function parseDecideApprovalRequest(
  ...
  if (!Number.isInteger(body.expectedVersion) || Number(body.expectedVersion) < 1) {
    throw new ApiError(400,"approval_version_invalid","expectedVersion must be a positive integer.");
  }
  return {
...
export function matchApprovalRequestPath(path: string): string | null {
  const match = path.match(/^\/api\/v1\/operations\/approval-requests\/([^/]+)$/);
  if (!match) return null;
  const id = decodeURIComponent(match[1]);
  if (!UUID.test(id)) throw new ApiError(400,"approval_request_id_invalid","Approval request id is invalid.");
  return id;
}

export function matchApprovalDecisionPath(path: string): { id:string; decision:ApprovalDecision } | null {
  const match = path.match(/^\/api\/v1\/operations\/approval-requests\/([^/]+)\/actions\/(approve|reject)$/);
  if (!match) return null;
  const id = decodeURIComponent(match[1]);
  if (!UUID.test(id)) throw new ApiError(400,"approval_request_id_invalid","Approval request id is invalid.");
...
export async function hashCreateApprovalRequest(payload: CreateApprovalRequest): Promise<string> {
  return await hash(["approval-request-v1",payload]);
}

export async function hashDecideApprovalRequest(id: string, payload: DecideApprovalRequest): Promise<string> {
