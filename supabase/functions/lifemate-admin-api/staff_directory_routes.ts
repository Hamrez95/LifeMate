...
import { createPrivacyLegalAdminRouteHandler } from "./privacy_legal_routes.ts";
...
export function createStaffDirectoryRouteHandler(databaseUrl: string) {
  ...
  const privacyLegalRouteHandler = createPrivacyLegalAdminRouteHandler(databaseUrl);
  ...
  return async function staffDirectoryRouteHandler(input: {
    ...
    const privacyLegalResponse = await privacyLegalRouteHandler(input);
    if (privacyLegalResponse) return privacyLegalResponse;
