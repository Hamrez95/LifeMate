...
Deno.test("RequireApproval is explicit and cannot omit approval policy", async () => {
  const error = await assertRejects(
    () => parseAbuseRuleMutation(request({
      ...
      priority: 10,
      expectedVersion: null,
      reason: "Route repeated refund requests through a configured approval policy.",
    })),
    ApiError,
  );
...
Deno.test("rule parameter shapes fail closed", async () => {
  const error = await assertRejects(
    () => parseAbuseRuleMutation(request({
      ...
      priority: 100,
      expectedVersion: null,
      reason: "Reject a malformed cooldown rule instead of silently ignoring it.",
    })),
    ApiError,
  );
...
Deno.test("retire route and payload require concurrency", async () => {
  ...
  assertEquals(
    matchAbuseRuleRetirePath(`/api/v1/security/abuse/rules/${id}/actions/retire`),
    id,
  );
...
Deno.test("abuse engine is explainable, idempotent and non-punitive", async () => {
  ...
  const hardening = await Deno.readTextFile(
    new URL("../../migrations/20260827023100_abuse_rule_hardening.sql", import.meta.url),
  );
  const idempotency = await Deno.readTextFile(
    new URL("../../migrations/20260827023200_abuse_rule_idempotency.sql", import.meta.url),
  );
  const priority = await Deno.readTextFile(
    new URL("../../migrations/20260827023300_abuse_evaluator_priority.sql", import.meta.url),
  );
  ...
  assertStringIncludes(hardening, "on conflict(context_code,subject_scope,subject_key_hash,operation_key_hash,event_code) do nothing");
  assertStringIncludes(idempotency, "admin.idempotency_keys");
  assertStringIncludes(priority, "v_rule.enforcement_action='RequireApproval' and v_final='Allow'");
...
Deno.test("abuse tables have no browser grants and edge uses functions only", async () => {
  ...
  assertStringIncludes(migration, "revoke all on table security.abuse_rules from public,anon,authenticated");
  assertStringIncludes(hardening, "revoke all on table security.abuse_events from lifemate_edge_runtime");
  assertStringIncludes(hardening, "revoke all on table security.abuse_decisions from lifemate_edge_runtime");
