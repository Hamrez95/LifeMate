...
Deno.test("abuse administration uses reachable high-risk RBAC and narrow writes", async () => {
  ...
  assertStringIncludes(hardening, "revoke insert,update on security.abuse_rules from lifemate_admin_runtime");
  assertStringIncludes(hardening, "revoke insert on security.abuse_rule_versions from lifemate_admin_runtime");
});
...
Deno.test("generic edge runtime cannot evaluate arbitrary subjects or record abuse events directly", async () => {
  ...
  assertStringIncludes(hardening, "revoke execute on function security.evaluate_abuse_rules");
  assertStringIncludes(hardening, "from lifemate_edge_runtime");
  assertStringIncludes(hardening, "revoke execute on function security.record_abuse_event");
});
