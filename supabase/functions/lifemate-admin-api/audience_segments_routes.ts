...
function segmentIdFromPath(path: string): string | null {
  ...
  if (!UUID.test(id)) throw new ApiError(400,"segment_id_invalid","Audience segment id is invalid.");
  return id;
}

function segmentActionId(path: string, action: "preview" | "snapshot"): string | null {
  const match = path.match(new RegExp(`^\\/api\\/v1\\/marketing\\/segments\\/([^/]+)\\/${action}$`));
  if (!match) return null;
  const id = decodeURIComponent(match[1]);
  if (!UUID.test(id)) throw new ApiError(400,"segment_id_invalid","Audience segment id is invalid.");
  return id;
...
function boundedText(value: unknown, field: string, max: number, required: boolean): string | null {
  if (value == null) {
    if (required) throw new ApiError(400,"segment_payload_invalid",`${field} is required.`);
    return null;
  }
  if (typeof value !== "string") throw new ApiError(400,"segment_payload_invalid",`${field} must be text.`);
  const text = value.trim();
  if (required && text.length < 2) throw new ApiError(400,"segment_payload_invalid",`${field} is too short.`);
  if (text.length > max) throw new ApiError(400,"segment_payload_invalid",`${field} is too long.`);
  return text || null;
}

async function parseWritePayload(request: Request, creating: boolean): Promise<SegmentWritePayload> {
  const body = await request.json().catch(() => null);
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    throw new ApiError(400,"segment_payload_invalid","Audience segment payload must be an object.");
  }
  const raw = body as Record<string, unknown>;
  let key: string | undefined;
  if (creating) {
    if (typeof raw.key !== "string" || !KEY.test(raw.key.trim().toLowerCase())) {
      throw new ApiError(400,"segment_key_invalid","Audience segment key is invalid.");
    }
  ...
  if (!creating) {
    if (!Number.isInteger(raw.expectedVersion) || Number(raw.expectedVersion) < 1) {
    ...
    if (raw.status !== "Active" && raw.status !== "Archived") {
      throw new ApiError(400,"segment_status_invalid","status must be Active or Archived.");
    }
  }
  return {
    key,
...
export function createAudienceSegmentRouteHandler(databaseUrl: string) {
  ...
  return async function handleAudienceSegmentRoute(context: RouteContext): Promise<Response | null> {
    const { request, path, accountId, admin, correlationId, origin } = context;

    if (request.method === "GET" && path === "/api/v1/marketing/segments/capabilities") {
    ...
    if (request.method === "GET" && path === "/api/v1/marketing/segments") {
      ...
      return json({items,total:items.length,freshness:{status:"fresh",asOfUtc:new Date().toISOString()}},200,origin);
    }
