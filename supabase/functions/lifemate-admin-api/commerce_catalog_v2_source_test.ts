...
Deno.test("catalog v2 routes stay behind purpose-specific admin permission", async () => {
  ...
  assertStringIncludes(routes, 'requirePermission(admin, "commerce.catalog.write")');
...
Deno.test("catalog v2 mutation migrations are audited idempotent and runtime narrow", async () => {
  const foundation = await source("../../migrations/20260827212000_commerce_catalog_v2_mutations.sql");
  const hardening = await source("../../migrations/20260827212100_commerce_catalog_v2_mutation_hardening.sql");
  const lifecycle = await source("../../migrations/20260827212200_commerce_catalog_v2_lifecycle_integrity.sql");
  ...
  assertStringIncludes(combined, "grant execute on function admin.update_commerce_catalog_product");
  assertStringIncludes(combined, "grant execute on function admin.update_commerce_catalog_bundle");
...
Deno.test("catalog lifecycle uses retirement instead of destructive catalog deletes", async () => {
  const lifecycle = await source("../../migrations/20260827212200_commerce_catalog_v2_lifecycle_integrity.sql");
  assertStringIncludes(lifecycle, "update commerce.bundles b set status='Retired'");
  assertStringIncludes(lifecycle, "update commerce.offers set status='Retired'");
...
Deno.test("bundle composition replacement is transactional and bounded", async () => {
  const migration = await source("../../migrations/20260827212000_commerce_catalog_v2_mutations.sql");
  assertStringIncludes(migration, "delete from commerce.bundle_items where bundle_id=p_bundle");
  assertStringIncludes(migration, "insert into commerce.bundle_items(bundle_id,offer_id,quantity)");
