...
Deno.test("update policy history stays read-only, permission-scoped, and bounded", async () => {
  const routes = await Deno.readTextFile(new URL("./product_version_analytics_routes.ts", import.meta.url));
  const service = await Deno.readTextFile(new URL("./product_version_analytics_service.ts", import.meta.url));

  assertEquals(routes.includes('path === "/api/v1/platform/product-update-policies/history"'), true);
  assertEquals(routes.includes('requirePermission(admin, "analytics.product_versions.read")'), true);
