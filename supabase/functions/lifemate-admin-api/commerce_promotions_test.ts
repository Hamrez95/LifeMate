...
function promotionRequest(body: Record<string, unknown>, method = "POST"): Request {
...
Deno.test("promotion routes keep detail and status actions distinct", () => {
  assertEquals(matchCommercePromotionDetailPath(`/api/v1/commerce/promotions/${promotionId}`), promotionId);
  assertEquals(matchCommercePromotionStatusPath(`/api/v1/commerce/promotions/${promotionId}/actions/status`), promotionId);
  assertEquals(matchCommercePromotionDetailPath(`/api/v1/commerce/promotions/${promotionId}/actions/status`), null);
});

Deno.test("discount-code routes bind promotion and code identifiers", () => {
  assertEquals(
    matchCommercePromotionCodesPath(`/api/v1/commerce/promotions/${promotionId}/discount-codes`),
    promotionId,
  );
  assertEquals(
    matchCommerceDiscountCodeStatusPath(
...
Deno.test("promotion list search prevents partial code enumeration", () => {
  const query = parseCommercePromotionsQuery(
    new URL("https://admin.test/api/v1/commerce/promotions?q=launch&code=welcome-125&status=Active&page=2&pageSize=20"),
  );
...
Deno.test("create parser normalizes codes and enforces bounded limits", async () => {
  ...
  await assertRejects(
    () => parseCreatePromotionPayload(promotionRequest({ ...percentageBody, maxRedemptions: 100, codeMaxRedemptions: 101 })),
...
Deno.test("promotion window and update code state are validated", async () => {
  await assertRejects(
    () => parseCreatePromotionPayload(
      promotionRequest({ ...percentageBody, endsAtUtc: "2026-08-14T09:00:00.000Z" }),
    ),
...
Deno.test("status mutation requires lifecycle state and meaningful reason", async () => {
  assertEquals(
    await parsePromotionStatusPayload(
      promotionRequest({ status: "Paused", reason: "Paused while finance reviews the promotion terms." }, "POST"),
    ),
    { status: "Paused", reason: "Paused while finance reviews the promotion terms." },
  );

  await assertRejects(
    () => parsePromotionStatusPayload(
      promotionRequest({ status: "Expired", reason: "Attempt to force lifecycle state directly." }, "POST"),
    ),
    "promotion_status_invalid",
  );
});

Deno.test("discount-code issuance supports explicit and bounded generated batches", async () => {
  assertEquals(
...
Deno.test("promotion mutation hashes are deterministic and resource scoped", async () => {
  const createPayload = await parseCreatePromotionPayload(promotionRequest(percentageBody));
  ...
  const statusPayload = await parsePromotionStatusPayload(
    promotionRequest({ status: "Active", reason: "Activate after commercial review is complete." }, "POST"),
  );
  ...
  const updateHash = await hashUpdatePromotionRequest(promotionId, updatePayload);
  ...
  const statusHash = await hashPromotionStatusRequest(promotionId, statusPayload);
  assert(statusHash !== updateHash, "action hashes must be operation scoped");
});

Deno.test("discount-code mutation hashes bind promotion, code and request", async () => {
  ...
  const issueHash = await hashIssueDiscountCodesRequest(promotionId, issuePayload);
  assertEquals(issueHash.length, 64);
  assertEquals(issueHash, await hashIssueDiscountCodesRequest(promotionId, issuePayload));
  ...
  const statusHash = await hashDiscountCodeStatusRequest(promotionId, codeId, statusPayload);
  const otherHash = await hashDiscountCodeStatusRequest(promotionId, "55555555-5555-4555-8555-555555555555", statusPayload);
  assert(statusHash !== otherHash, "discount-code status hash must bind code id");
  assert(statusHash !== issueHash, "discount-code operations must be hash scoped");
});
