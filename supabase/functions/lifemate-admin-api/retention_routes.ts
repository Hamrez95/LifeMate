...
function boundedString(value: unknown, field: string, min: number, max: number): string {
  ...
  if (trimmed.length < min || trimmed.length > max) throw new ApiError(400, `${field}_invalid`, `${field} is invalid.`);
  return trimmed;
}
...
function mutationStatus(result: Record<string, unknown>): number {
  ...
  if (!Number.isInteger(status) || status < 100 || status > 599) {
    throw new ApiError(503, "retention_workflow_unavailable", "Retention workflow returned an invalid status.");
  }
  if (status >= 400) throw new ApiError(status, String(result.code), typeof result.message === "string" ? result.message : "Retention operation failed.");
  return status;
}
...
export function createRetentionRouteHandler(databaseUrl: string) {
  ...
  return async function handleRetentionRoute(context: Context): Promise<Response | null> {
    ...
    if (request.method === "GET" && path === "/api/v1/security/retention/policies") {
    ...
    if (request.method === "GET" && path === "/api/v1/security/retention/deletion-preview") {
      requirePermission(admin, "security.retention.read");
      const rows = await sql`
        select
    ...
    if (request.method === "GET" && path === "/api/v1/security/retention/holds") {
      requirePermission(admin, "security.retention.read");
      const url = new URL(request.url);
      const accountIdFilter = url.searchParams.get("accountId");
      if (accountIdFilter && !uuidPattern.test(accountIdFilter)) throw new ApiError(400, "account_id_invalid", "accountId is invalid.");
    ...
    if (request.method === "POST" && path === "/api/v1/security/retention/policies") {
      ...
      if (!category) throw new ApiError(400, "data_category_invalid", "dataCategory is required.");
      const retentionDays = integer(body.retentionDays, "retention_days", 0, 36500, true);
      const graceDays = integer(body.graceDays ?? 0, "grace_days", 0, 3650) ?? 0;
    ...
    if (request.method === "POST" && path === "/api/v1/security/retention/holds") {
      ...
      if (!reasonCode) throw new ApiError(400, "reason_code_invalid", "reasonCode is required.");
      const reason = boundedString(body.reason, "reason", 10, 1000);
