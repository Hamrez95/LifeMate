    ...
    if (request.method === "POST" || request.method === "PUT") {
      ...
      if (scenarioId !== undefined) {
        ...
        if (request.method === "POST" && payload.expectedVersion !== null) {
          throw new ApiError(400, "finance_scenario_version_invalid", "New scenarios must not provide an expected version.");
        }
        if (request.method === "PUT" && payload.expectedVersion === null) {
          throw new ApiError(400, "finance_scenario_version_required", "Existing scenarios require an expected version.");
        }
