...
function requiredEnum<T extends string>(value: unknown, allowed: readonly T[], code: string): T {
...
function version(value: unknown, nullable: boolean): string | null {
  ...
  if (typeof value !== "string" || !VERSION.test(value)) {
    throw new ApiError(400, "update_policy_version_invalid", "Version value is invalid.");
  }
  return value;
}
...
export async function parseProductUpdatePolicyMutation(
  ...
  const product = requiredEnum(body.product, ["wellmate", "caremate"] as const, "product_invalid");
  ...
  const mode = requiredEnum(body.mode, ["Soft", "Force"] as const, "update_mode_invalid");
  const reasonCode = requiredEnum(
    body.reasonCode,
    ["Routine", "Critical", "Security", "BreakingCompatibility"] as const,
    "update_reason_invalid",
  );
  if (mode === "Force" && !["Critical", "Security", "BreakingCompatibility"].includes(reasonCode)) {
    throw new ApiError(400, "update_mode_invalid", "Force update requires a critical, security, or compatibility reason.");
  }
  const status = requiredEnum(body.status, ["Active", "Disabled"] as const, "status_invalid");
  const messageKey = body.messageKey == null || body.messageKey === "" ? null : body.messageKey;
  if (messageKey !== null && (typeof messageKey !== "string" || !MESSAGE_KEY.test(messageKey))) {
    throw new ApiError(400, "message_key_invalid", "Message key is invalid.");
  }
  if (typeof body.effectiveAtUtc !== "string") {
    throw new ApiError(400, "effective_at_invalid", "effectiveAtUtc is required.");
  }
  const effective = new Date(body.effectiveAtUtc);
  if (Number.isNaN(effective.getTime())) {
    throw new ApiError(400, "effective_at_invalid", "effectiveAtUtc is invalid.");
  }
  if (!Number.isInteger(body.expectedVersion) || Number(body.expectedVersion) < 0) {
    throw new ApiError(400, "expected_version_invalid", "expectedVersion must be zero or greater.");
  }
  if (typeof body.reason !== "string" || body.reason.trim().length < 10 || body.reason.trim().length > 1000) {
    throw new ApiError(400, "reason_invalid", "Reason must contain between 10 and 1000 characters.");
  }
  return {
    product,
    platform,
