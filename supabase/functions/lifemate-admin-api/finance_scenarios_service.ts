...
function mutationResult(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new ApiError(
      ...
      "Finance scenario workflow result was unavailable.",
    );
  }
  ...
  if (
    ...
    throw new ApiError(
      503,
      "finance_scenario_workflow_unavailable",
      "Finance scenario workflow result was invalid.",
    );
  }
  return row;
}
...
function scenarioRow(row: Record<string, unknown>) {
  ...
  if (!Array.isArray(assumptions)) {
    throw new ApiError(
      503,
      "finance_scenario_data_invalid",
      "Finance scenario data was invalid.",
    );
  }
  return {
