...
Deno.test("retention v3 extends existing lifecycle model without browser grants", async () => {
  ...
  const hardening = await Deno.readTextFile(
    new URL("../../migrations/20260827021400_retention_security_hardening.sql", import.meta.url),
  );
  ...
  assertStringIncludes(hardening, "revoke insert,update on security.retention_holds from lifemate_admin_runtime");
...
Deno.test("retention admin API is permissioned, bounded and idempotent", async () => {
  ...
  const idempotency = await Deno.readTextFile(
    new URL("../../migrations/20260827021500_retention_idempotent_mutations.sql", import.meta.url),
  );
