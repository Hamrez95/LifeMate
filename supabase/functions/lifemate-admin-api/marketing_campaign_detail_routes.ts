...
export function createMarketingCampaignDetailRouteHandler(databaseUrl: string) {
  ...
  return async function handle(context: MarketingCampaignDetailRouteContext): Promise<Response | null> {
    ...
    if (request.method === "GET" && readId) {
      ...
      if (!detail) throw new ApiError(404, "marketing_campaign_not_found", "Campaign was not found.");
    ...
    if (request.method === "PUT" && contentId) {
      ...
      const result = await store.updateContent({ actorAccountId: accountId, campaignId: contentId, payload, correlationId, idempotencyKey, requestHash });
      const status = mutationStatus(result);
      if (status >= 400) throw new ApiError(status, String(result.code), mutationErrorMessage(result, "Campaign content update was not completed."));
      return json({
    ...
    if (request.method === "POST" && approvalId) {
      ...
      const requestHash = await hashCampaignApprovalRequest(approvalId, payload);
      const result = await store.setApproval({ actorAccountId: accountId, campaignId: approvalId, payload, correlationId, idempotencyKey, requestHash });
      const status = mutationStatus(result);
      if (status >= 400) throw new ApiError(status, String(result.code), mutationErrorMessage(result, "Campaign approval was not completed."));
      return json({
        campaignId: String(result.campaignId),
        contentRevision: Number(result.contentRevision),
        approvalState: String(result.approvalState),
    ...
    if (request.method === "POST" && publishId) {
      ...
      const result = await store.requestPublish({ actorAccountId: accountId, campaignId: publishId, payload, correlationId, idempotencyKey, requestHash });
      const status = mutationStatus(result);
      if (status >= 400) throw new ApiError(status, String(result.code), mutationErrorMessage(result, "Campaign publish request was not completed."));
      return json({
        campaignId: String(result.campaignId),
