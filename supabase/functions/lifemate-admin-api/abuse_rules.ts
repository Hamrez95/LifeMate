...
const kinds = new Set(["VelocityLimit", "UsageCap", "Cooldown", "DuplicateKey", "EvidenceRequired"]);
...
function key(value: unknown, field: string, nullable = false): string | null {
  if (nullable && (value === null || value === undefined || value === "")) return null;
  const result = text(value, field, 3, 80).toLowerCase();
  if (!keyPattern.test(result)) throw new ApiError(400, `${field}_invalid`, `${field} is invalid.`);
  return result;
}
...
export async function parseAbuseRuleMutation(request: Request): Promise<AbuseRuleMutation> {
  ...
  const enforcementAction = text(body.enforcementAction, "enforcement_action", 4, 24);
  if (!kinds.has(ruleKind)) throw new ApiError(400, "rule_kind_invalid", "ruleKind is invalid.");
  if (!scopes.has(subjectScope)) throw new ApiError(400, "subject_scope_invalid", "subjectScope is invalid.");
  if (!actions.has(enforcementAction)) throw new ApiError(400, "enforcement_action_invalid", "enforcementAction is invalid.");

  const windowSeconds = integer(body.windowSeconds, "window_seconds", 1, 31536000, true);
  const maxCount = integer(body.maxCount, "max_count", 1, 1000000, true);
  const cooldownSeconds = integer(body.cooldownSeconds, "cooldown_seconds", 1, 31536000, true);
  const evidenceCode = key(body.evidenceCode, "evidence_code", true);
  const approvalRequestType = key(body.approvalRequestType, "approval_request_type", true);
  const priority = integer(body.priority ?? 100, "priority", 1, 10000)!;
  const expectedVersion = integer(body.expectedVersion, "expected_version", 1, Number.MAX_SAFE_INTEGER, true);
  ...
  if (enforcementAction === "RequireApproval" ? !approvalRequestType : approvalRequestType !== null) {
    throw new ApiError(400, "approval_request_type_invalid", "RequireApproval must name exactly one approvalRequestType.");
  }
  const shapeValid =
  ...
  if (!shapeValid) throw new ApiError(400, "abuse_rule_shape_invalid", "Rule parameters do not match ruleKind.");
  ...
  return {
    ...
    enforcementAction: enforcementAction as AbuseRuleMutation["enforcementAction"],
    windowSeconds,
    maxCount,
    cooldownSeconds,
    evidenceCode,
...
export async function parseAbuseRuleRetire(request: Request): Promise<AbuseRuleRetire> {
  const body = await objectBody(request);
  return {
    expectedVersion: integer(body.expectedVersion, "expected_version", 1, Number.MAX_SAFE_INTEGER)! ,
    reason: text(body.reason, "reason", 10, 1000),
  };
}

export function matchAbuseRuleRetirePath(path: string): string | null {
  const match = path.match(/^\/api\/v1\/security\/abuse\/rules\/([0-9a-f-]{36})\/actions\/retire$/i);
  return match?.[1] ?? null;
}
...
export async function hashAbuseRuleMutation(payload: AbuseRuleMutation): Promise<string> {
  return sha256(stable(payload));
}

export async function hashAbuseRuleRetire(id: string, payload: AbuseRuleRetire): Promise<string> {
