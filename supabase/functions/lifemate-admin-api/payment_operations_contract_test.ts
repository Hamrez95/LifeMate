...
Deno.test("payment operations preserve provider facts and expose truthful read models", async () => {
  const foundation = await Deno.readTextFile(
    new URL("../../migrations/20260827032000_payment_refund_churn_operations.sql", import.meta.url),
  );
  const routes = await Deno.readTextFile(new URL("./payment_operations_routes.ts", import.meta.url));
...
Deno.test("reconciliation corrections are append-only and v1 executor is revoked", async () => {
  const reconciliation = await Deno.readTextFile(
    new URL("../../migrations/20260827032200_reconciliation_and_churn_contracts.sql", import.meta.url),
  );
  const idempotency = await Deno.readTextFile(
    new URL("../../migrations/20260827032300_payment_operations_idempotency.sql", import.meta.url),
  );
  const cleanup = await Deno.readTextFile(
    new URL("../../migrations/20260827032500_payment_operations_entrypoint_cleanup.sql", import.meta.url),
  );
  assertStringIncludes(reconciliation, "insert into commerce.transaction_corrections");
  assertStringIncludes(idempotency, "apply_approved_transaction_correction_v2");
  assertStringIncludes(cleanup, "revoke all on function admin.apply_approved_transaction_correction");
  assert(!reconciliation.includes("update commerce.transactions set normalized_status"));
});
...
Deno.test("new operations stay behind split permissions and server-only routes", async () => {
  ...
  assertStringIncludes(routes, 'requirePermission(admin, "commerce.refund.read")');
  assertStringIncludes(routes, 'requirePermission(admin, "commerce.reconciliation.read")');
  assertStringIncludes(routes, 'requirePermission(admin, "commerce.churn.read")');
