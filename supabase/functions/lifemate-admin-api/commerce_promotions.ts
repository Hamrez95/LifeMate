...
function parseFixedAmount(value: unknown): string {
  if (typeof value !== "string" || !AMOUNT_PATTERN.test(value)) {
    throw new ApiError(
      ...
      "Fixed amount discount is invalid.",
    );
  }
  ...
  if (parsed <= 0n || parsed > POSTGRES_BIGINT_MAX) {
    throw new ApiError(
      400,
      "promotion_discount_invalid",
      "Fixed amount discount is outside the supported range.",
    );
  }
...
function parseSharedPayload(
  ...
  const primaryCode = requiredText(
    body.primaryCode,
    3,
    64,
  ...
  const startsAtUtc = requiredInstant(
    body.startsAtUtc,
  ...
  if (type === "Percentage") {
    ...
    if (
      ...
      throw new ApiError(
        400,
        "promotion_discount_invalid",
        "Percentage discount is invalid.",
      );
    }
    ...
    if (body.percentageBasisPoints != null) {
      throw new ApiError(
        400,
        "promotion_discount_invalid",
        "Fixed-amount promotion cannot include percentageBasisPoints.",
      );
    }
  ...
  const codeMaxRedemptions = optionalPositiveInteger(
    body.codeMaxRedemptions,
    "promotion_limit_invalid",
  );
  if (
    maxRedemptions != null &&
    ...
    throw new ApiError(
      400,
      "promotion_limit_invalid",
      "Discount-code limit cannot exceed promotion limit.",
    );
  }
...
export function parseCommercePromotionsQuery(
  url: URL,
  ...
  if (
    ...
    throw new ApiError(
      ...
      "Promotion status filter is invalid.",
    );
  }
  ...
  if (qRaw.length > 80) {
    throw new ApiError(
      400,
      "invalid_promotion_query",
      "Promotion search is too long.",
    );
  }
  if (codeRaw && !CODE_PATTERN.test(codeRaw)) {
    throw new ApiError(
      400,
      "invalid_discount_code_filter",
      "Exact discount-code filter is invalid.",
    );
  }
  return {
...
export async function parseUpdatePromotionPayload(
  request: Request,
...
export async function parsePromotionStatusPayload(
  request: Request,
): Promise<PromotionStatusPayload> {
  const body = await requestObject(request);
  if (body.status !== "Active" && body.status !== "Paused") {
    throw new ApiError(
      400,
      "promotion_status_invalid",
      "Target promotion status is invalid.",
    );
  }
  return {
    status: body.status,
...
export async function hashUpdatePromotionRequest(
  promotionId: string,
