...
function positiveInt(value: unknown, field: string, max: number): number | null {
...
function iso(value: unknown, field: string, required: boolean): string | null {
  if (value == null || value === "") {
    if (required) throw new ApiError(400, `${field}_invalid`, `${field} is required.`);
    return null;
  }
  if (typeof value !== "string") throw new ApiError(400, `${field}_invalid`, `${field} is invalid.`);
  const parsed = new Date(value);
  if (!Number.isFinite(parsed.getTime())) throw new ApiError(400, `${field}_invalid`, `${field} is invalid.`);
...
export async function parseManualEntitlementPayload(
  ...
  if (!operations.has(operation)) throw new ApiError(400, "operation_invalid", "operation is invalid.");
  if (!schedules.has(scheduleMode)) throw new ApiError(400, "schedule_mode_invalid", "scheduleMode is invalid.");
  ...
  if (operation === "Grant" && (entitlementId || expectedEntitlementVersion)) {
    throw new ApiError(400, "grant_existing_invalid", "Grant must not include an existing entitlement.");
  }
  if (operation !== "Grant" && (!entitlementId || !expectedEntitlementVersion)) {
    throw new ApiError(400, "entitlement_version_required", "Existing adjustments require entitlementId and expectedEntitlementVersion.");
  }

  const maxAmount = scheduleMode === "AddMonths" ? 120 : 3650;
  const scheduleAmount = positiveInt(body.scheduleAmount, "schedule_amount", maxAmount);
  const exactExpiresAtUtc = iso(body.exactExpiresAtUtc, "exact_expires_at_utc", false);
  if ((scheduleMode === "AddDays" || scheduleMode === "AddMonths") && !scheduleAmount) {
    throw new ApiError(400, "schedule_amount_required", "Selected schedule requires scheduleAmount.");
  }
  if (scheduleMode === "ExactExpiry" && !exactExpiresAtUtc) {
    throw new ApiError(400, "exact_expiry_required", "ExactExpiry requires exactExpiresAtUtc.");
  }
  if (scheduleMode === "Immediate" && operation !== "Revoke") {
    throw new ApiError(400, "immediate_mode_invalid", "Immediate mode is reserved for Revoke.");
  }
  if (operation === "Reduce" && scheduleMode !== "ExactExpiry") {
    throw new ApiError(400, "reduce_schedule_invalid", "Reduce requires ExactExpiry.");
  }
  ...
  if (options.requireConfirmation && (operation === "Reduce" || operation === "Revoke") && !confirmed) {
    throw new ApiError(400, "confirmation_required", "Reduce and Revoke require explicit confirmation.");
  }

  const approvalRequestId = optionalUuid(body.approvalRequestId, "approval_request_id");
  ...
  if ((approvalRequestId == null) !== (approvalExpectedVersion == null)) {
    throw new ApiError(400, "approval_pair_invalid", "Approval id and expected version must be supplied together.");
  }
...
export async function hashManualEntitlementPayload(payload: ManualEntitlementPayload): Promise<string> {
...
export function matchManualEntitlementHistoryPath(path: string): string | null {
  const match = path.match(/^\/api\/v1\/commerce\/accounts\/([^/]+)\/entitlement-adjustments$/i);
