...
Deno.test(
  "missing engagement age stays unknown instead of becoming fabricated inactivity",
  async () => {
    const text = await source("./audience_segments_service.ts");
...
Deno.test(
  "audience migration is browser-denied, portable, deletion-compatible and history append-only",
  async () => {
    ...
    assert(!migration.includes("grant select,insert on audience.segment_history"));
    assert(
      !migration.includes(
...
Deno.test(
  "execution snapshots validate and persist under one database transaction",
  async () => {
...
Deno.test(
  "audience segment workflow uses canonical idempotency and audit enum values",
  async () => {
    const text = await source("./audience_segments_service.ts");
