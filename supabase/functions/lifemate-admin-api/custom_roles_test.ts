...
function request(body:unknown):Request {
  return new Request("https://example.test",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});
}

Deno.test("custom role create request is bounded and canonical", async () => {
  const parsed=await parseCustomRoleMutationRequest(request({
    code:"support_lead",
...
Deno.test("founder and super admin role codes stay reserved", async () => {
  for (const code of ["founder","super_admin"]) {
    const error=await assertRejects(() => parseCustomRoleMutationRequest(request({
      code,displayName:"Reserved",rank:200,reason:"Attempt a reserved administrative role code.",
    }),"create"),ApiError);
...
Deno.test("custom role permission request requires concurrency and reason", async () => {
  ...
  const hash=await hashCustomRolePermissionRequest("support_lead","assign",parsed);
  assert(/^[0-9a-f]{64}$/.test(hash));
});

Deno.test("custom role route matching is explicit", () => {
  assertEquals(matchCustomRolePath("/api/v1/security/custom-roles/support_lead"),"support_lead");
  assertEquals(matchCustomRoleRetirePath("/api/v1/security/custom-roles/support_lead/actions/retire"),"support_lead");
  assertEquals(matchCustomRolePermissionPath("/api/v1/security/custom-roles/support_lead/permissions/assign"),{roleCode:"support_lead",action:"assign"});
  assertEquals(matchCustomRolePermissionPath("/api/v1/security/custom-roles/support_lead/permissions/revoke"),{roleCode:"support_lead",action:"revoke"});
});
...
Deno.test("custom role routes use purpose-specific write permission and no browser database path", async () => {
  const routes=await Deno.readTextFile(new URL("./custom_roles_routes.ts",import.meta.url));
  const service=await Deno.readTextFile(new URL("./custom_roles_service.ts",import.meta.url));
  assertStringIncludes(routes,'requirePermission(admin,"security.roles.write")');
  assertStringIncludes(routes,'requirePermission(admin,"security.audit.read")');
