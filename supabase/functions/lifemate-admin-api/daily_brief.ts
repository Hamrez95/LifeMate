...
function recentChange(value: KpiValue, evidenceId: string): DailyBriefItem | null {
  ...
  if (recent === previous) {
    return {
      ...
      detail: `مجموع ۷ روز اخیر ${recent} و ۷ روز قبل نیز ${previous} بوده است. این مقایسه مستقیماً از series canonical ساخته شده است.`,
  ...
  return {
    ...
    detail: `مجموع ۷ روز اخیر ${recent} در برابر ${previous} در ۷ روز قبل است. درصد تغییر عمداً در صورت مبنای صفر محاسبه نمی‌شود.`,
    evidenceIds: [evidenceId],
  };
}

export function buildDailyBrief(values: readonly KpiValue[], generatedAtUtc: string): DailyBrief {
  const allowed = values.filter((value) =>
    value.name === "accounts_created" || value.name === "monthly_active_accounts"
  );
  ...
  for (const value of allowed) {
    ...
    if (value.state === "unavailable" || value.value === null) {
      attention.push({
        ...
        detail: "منبع canonical مقدار قابل اتکایی برنگردانده است؛ Daily Brief به‌جای حدس، این مورد را unavailable نگه داشته است.",
        evidenceIds: [evidenceId],
      });
      actions.push({
        ...
        detail: "این یک اقدام پیشنهادی برای بررسی انسانی read model است و هیچ mutation خودکاری اجرا نمی‌کند.",
        evidenceIds: [evidenceId],
      });
    } else if (value.state === "partial" || value.freshness.status !== "fresh") {
      attention.push({
        id: `attention:${value.name}:partial`,
        severity: "attention",
        title: `${label(value.name)} با caveat داده همراه است`,
        detail: value.reason ?? "وضعیت freshness این KPI کامل نیست و باید با احتیاط تفسیر شود.",
        evidenceIds: [evidenceId],
      });
    }
  }
  ...
  if (evidenceItems.length === 0) {
    return {
      ...
      attention: [{
        ...
        detail: "هیچ KPI allowlisted از read modelهای approved بازنگشت؛ خروجی مدیریتی ساخته یا حدس زده نشد.",
        evidenceIds: [],
      }],
      actions: [],
      evidence: [],
      caveats: ["Daily Brief فقط read-only است و هیچ اقدام خودکاری اجرا نمی‌کند."],
      generatedAtUtc,
    };
  }
  ...
  const state = evidenceItems.some((item) => item.state === "unavailable")
    ? "partial"
    : evidenceItems.some((item) => item.state === "partial" || item.freshness.status !== "fresh")
      ? "partial"
      : "ready";
