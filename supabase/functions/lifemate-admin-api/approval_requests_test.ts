...
Deno.test("approval decisions require optimistic version and explicit reason", async () => {
  ...
  const error=await assertRejects(
    () => parseDecideApprovalRequest(request({expectedVersion:0,reason:"Long enough reason text."}),"reject"),
    ApiError,
  );
...
Deno.test("approval route matching is explicit", () => {
  ...
  assertEquals(matchApprovalRequestPath(`/api/v1/operations/approval-requests/${id}`),id);
  assertEquals(matchApprovalDecisionPath(`/api/v1/operations/approval-requests/${id}/actions/approve`),{id,decision:"approve"});
  assertEquals(matchApprovalDecisionPath(`/api/v1/operations/approval-requests/${id}/actions/reject`),{id,decision:"reject"});
});
...
Deno.test("approval ledger is server-only, purpose-scoped and self-approval defaults denied", async () => {
  ...
  assertStringIncludes(migration,"self_approval_allowed boolean not null default false");
...
Deno.test("generic approval API cannot execute a child-domain mutation by itself", async () => {
  ...
  const migration=await Deno.readTextFile(new URL("../../migrations/20260827013000_admin_approval_adjustment_ledger.sql",import.meta.url));
  assertStringIncludes(migration,"create or replace function admin.consume_approval_request");
  assertStringIncludes(migration,"if not admin.account_has_permission(p_actor_account_id,v_policy.execution_permission)");
});
