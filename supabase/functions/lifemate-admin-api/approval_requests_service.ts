...
function mutationResult(value: unknown): Row {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new ApiError(503,"approval_workflow_unavailable","Approval workflow result was unavailable.");
  }
  const row=value as Row;
  if (!Number.isInteger(row.httpStatus) || typeof row.code!=="string" || typeof row.replayed!=="boolean") {
    throw new ApiError(503,"approval_workflow_unavailable","Approval workflow result was invalid.");
  }
  return row;
}
...
function requestRow(row: Row) {
  return {
    ...
    reviewedByAccountId:row.reviewed_by_account_id ? String(row.reviewed_by_account_id) : null,
    reviewedAtUtc:row.reviewed_at_utc ? new Date(String(row.reviewed_at_utc)).toISOString() : null,
    reviewReason:row.review_reason ? String(row.review_reason) : null,
    executedByAccountId:row.executed_by_account_id ? String(row.executed_by_account_id) : null,
    executedAtUtc:row.executed_at_utc ? new Date(String(row.executed_at_utc)).toISOString() : null,
    executionOperation:row.execution_operation ? String(row.execution_operation) : null,
...
export function createApprovalRequestStore(databaseUrl:string) {
  ...
  return {
    async list(status:string|null,requestType:string|null,limit:number) {
