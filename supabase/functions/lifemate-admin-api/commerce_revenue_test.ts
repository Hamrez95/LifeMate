...
Deno.test("commerce revenue parser rejects invalid ranges and currency", () => {
  ...
  try {
    parseCommerceRevenueQuery(
      new URL("https://example.test/api/v1/commerce/revenue?from=2026-08-26&to=2026-08-01"),
    );
  } catch {
    reversed = true;
  }
...
Deno.test("recurring revenue metrics never infer values from current prices", () => {
  ...
  assert(metrics.length === 5, "five unsupported recurring KPIs must be explicit");
  assert(metrics.every((metric) => metric.state === "unavailable"), "unsupported KPIs must fail closed");
  assert(metrics.every((metric) => metric.value === null), "unsupported KPIs must never expose invented values");
  assert(
    metrics.find((metric) => metric.name === "mrr")?.reason.includes("price × subscriber count"),
