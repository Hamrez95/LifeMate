...
const unavailableMetrics: MarketingAttributionMetric[] = ["spend", "revenue", "conversions", "cac", "roas"].map((name) => ({
...
export function createMarketingAttributionStore(databaseUrl: string) {
  ...
  return {
    async read(query: MarketingAttributionQuery) {
      const rows = await sql<Array<{ product_code: string | null; channel_code: string | null; campaign_count: number | string; active_campaign_count: number | string; completed_campaign_count: number | string }>>`
        select c.product_code, c.channel_code,
