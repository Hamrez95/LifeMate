...
Deno.test("matches only canonical staff detail paths", () => {
  ...
  assertEquals(matchStaffDetailPath(`/api/v1/staff/${ACCOUNT_ID}/roles/assign`), null);
});

Deno.test("parses bounded safe staff directory filters", () => {
  const query = parseStaffDirectoryQuery(
    new URL("https://admin.example/api/v1/staff?pageSize=50&status=Active&role=support&q=ali"),
  );
...
Deno.test("round-trips opaque stable staff directory cursor", () => {
  ...
  const query = parseStaffDirectoryQuery(
    new URL(`https://admin.example/api/v1/staff?cursor=${encodeURIComponent(cursor)}`),
  );
...
Deno.test("rejects unsafe staff directory filters and malformed cursors", () => {
  assertThrows(
    () => parseStaffDirectoryQuery(new URL("https://admin.example/api/v1/staff?status=Pending")),
    ApiError,
  );
  assertThrows(
    () => parseStaffDirectoryQuery(new URL("https://admin.example/api/v1/staff?role=SUPER-ADMIN!")),
    ApiError,
  );
  assertThrows(
    () => parseStaffDirectoryQuery(new URL("https://admin.example/api/v1/staff?q=x")),
    ApiError,
  );
  assertThrows(
    () => parseStaffDirectoryQuery(new URL("https://admin.example/api/v1/staff?cursor=not-a-cursor")),
    ApiError,
  );
});
