...
function status(result: Record<string, unknown>): number {
  ...
  if (!Number.isInteger(value) || value<100 || value>599) {
    throw new ApiError(503,"custom_role_workflow_unavailable","Custom role workflow returned an invalid status.");
  }
  return value;
}
...
function requireSuccess(result: Record<string, unknown>): number {
  ...
  if (httpStatus>=400) {
    throw new ApiError(
      httpStatus,
      String(result.code),
      typeof result.message === "string" ? result.message : "Custom role mutation was not completed.",
    );
  }
  return httpStatus;
}
...
export function createCustomRoleRouteHandler(databaseUrl:string) {
  ...
  return async function handleCustomRoleRoute(context:Context):Promise<Response | null> {
    ...
    if (request.method==="GET" && path==="/api/v1/security/custom-roles") {
      ...
      return json({...data,freshness:{status:"fresh",asOfUtc:new Date().toISOString()}},200,origin);
    }
    ...
    if (request.method==="POST" && retireCode) {
      requirePermission(admin,"security.roles.write");
      const idempotencyKey=requireIdempotencyKey(request);
      const payload=await parseCustomRoleMutationRequest(request,"retire",retireCode);
      const result=await store.mutateRole({
        actorAccountId:accountId,
        action:"retire",
        payload,
    ...
    if (request.method==="POST" && permissionRoute) {
      ...
      const result=await store.mutatePermission({
        ...
        correlationId,
        idempotencyKey,
        requestHash:await hashCustomRolePermissionRequest(permissionRoute.roleCode,permissionRoute.action,payload),
      });
      return json(result,requireSuccess(result),origin);
    }
    ...
    if (request.method==="PUT" && roleCode) {
      requirePermission(admin,"security.roles.write");
      const idempotencyKey=requireIdempotencyKey(request);
      const payload=await parseCustomRoleMutationRequest(request,"update",roleCode);
      const result=await store.mutateRole({
        actorAccountId:accountId,
        action:"update",
        payload,
