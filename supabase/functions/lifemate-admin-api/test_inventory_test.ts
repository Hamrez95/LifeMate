...
Deno.test("all Admin API unit tests are type-checked and executed", async () => {
  ...
  assertEquals(
    ...
    [],
    "Admin API unit tests must be executed by deno task test.",
  );
  assertEquals(
    unitTests.filter((name) => !checkTokens.has(name)),
    [],
    "Admin API unit tests must be type-checked by deno task check.",
  );
});
