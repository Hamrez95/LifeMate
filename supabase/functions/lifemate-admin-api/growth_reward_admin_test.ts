...
Deno.test("growth reward admin contracts are bounded and deterministic", async () => {
  const rule = await parseRewardRuleMutation(new Request("https://admin.test/api/v1/commerce/rewards/rules", {
    method: "POST",
    body: JSON.stringify({
  ...
  const fulfillment = await parseRewardFulfillmentExecute(new Request("https://admin.test/api/v1/commerce/rewards/fulfill", {
    method: "POST",
    body: JSON.stringify({
  ...
  assertEquals(
    matchRewardSourceReviewPath("/api/v1/commerce/rewards/sources/Advocacy/44444444-4444-4444-8444-444444444444/review"),
    { sourceKind: "Advocacy", sourceId: "44444444-4444-4444-8444-444444444444" },
  );
  assertEquals(await hashGrowthRewardAdminRequest(fulfillment), await hashGrowthRewardAdminRequest({ ...fulfillment }));
});

Deno.test("growth reward admin rejects oversized and malformed mutation input", async () => {
  await assertRejects(
    () => parseRewardRuleMutation(new Request("https://admin.test", {
      method: "POST",
...
Deno.test("reward fulfillment consumes canonical approval and reuses guarded entitlement mutation", async () => {
  const migration = await Deno.readTextFile(new URL("../../migrations/20260827041600_growth_reward_review_fulfillment.sql", import.meta.url));
...
Deno.test("reward administration remains behind existing Admin RBAC dispatcher", async () => {
  const router = await Deno.readTextFile(new URL("./commerce_catalog_routes.ts", import.meta.url));
  const routes = await Deno.readTextFile(new URL("./growth_reward_admin_routes.ts", import.meta.url));
  ...
  assertStringIncludes(routes, 'requirePermission(admin, "growth.rewards.write")');
  assertStringIncludes(routes, 'requirePermission(admin, "growth.rewards.read")');
