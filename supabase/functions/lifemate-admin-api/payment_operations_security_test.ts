...
Deno.test("generic edge runtime cannot spoof renewal-intent actors", async () => {
  const migration = await Deno.readTextFile(
    new URL("../../migrations/20260827032700_payment_operations_security_boundary.sql", import.meta.url),
  );
  assertStringIncludes(migration, "revoke execute on function commerce.set_subscription_renewal_intent_v2");
