...
function status(result: Record<string, unknown>): number {
  ...
  if (!Number.isInteger(value) || value < 100 || value > 599) {
    throw new ApiError(503, "commerce_catalog_v2_workflow_unavailable", "Commerce catalog v2 mutation returned an invalid status.");
  }
  return value;
}
...
function respond(result: Record<string, unknown>, origin: string | null) {
  ...
  if (httpStatus >= 400) {
    throw new ApiError(
      httpStatus,
      typeof result.code === "string" ? result.code : "commerce_catalog_v2_mutation_failed",
      typeof result.message === "string" ? result.message : "Commerce catalog v2 mutation was not completed.",
    );
  }
...
export function createCommerceCatalogV2MutationRouteHandler(databaseUrl: string) {
  ...
  return async function handler(input: {
    ...
    if (request.method === "PUT" && productId) {
      ...
      return respond(await store.updateProduct({
        actorAccountId: accountId,
        productId,
        payload,
        correlationId,
        idempotencyKey,
        requestHash: await hashCatalogMutation("commerce.catalog.product.update", productId, payload),
      }), origin);
    }

    if (request.method === "POST" && isCatalogOfferCreatePath(path)) {
      const payload = await parseCreateCatalogOffer(request);
      return respond(await store.createOffer({
        actorAccountId: accountId,
        payload,
        correlationId,
        idempotencyKey,
        requestHash: await hashCatalogMutation("commerce.catalog.offer.create", null, payload),
      }), origin);
    }
    ...
    if (request.method === "POST" && offerPriceId) {
      ...
      return respond(await store.scheduleOfferPrice({
        actorAccountId: accountId,
        offerId: offerPriceId,
        payload,
        correlationId,
        idempotencyKey,
        requestHash: await hashCatalogMutation("commerce.catalog.offer.price.schedule", offerPriceId, payload),
      }), origin);
    }
    ...
    if (request.method === "PUT" && offerId) {
      ...
      return respond(await store.updateOffer({
        actorAccountId: accountId,
        offerId,
        payload,
        correlationId,
        idempotencyKey,
        requestHash: await hashCatalogMutation("commerce.catalog.offer.update", offerId, payload),
      }), origin);
    }
    ...
    if (request.method === "PUT" && policy) {
      ...
      return respond(await store.upsertPolicy({
        actorAccountId: accountId,
        ...
        correlationId,
        idempotencyKey,
        requestHash: await hashCatalogMutation("commerce.catalog.policy.upsert", `${policy.productId}:${policy.policyKey}`, payload),
      }), origin);
    }
