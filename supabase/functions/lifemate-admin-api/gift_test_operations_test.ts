...
Deno.test("gift test finalize rejects raw or malformed claim tokens", async () => {
  await assertRejects(
    () => parseGiftTestFinalizePayload(new Request("https://example.test", {
      method: "POST",
      body: JSON.stringify({
        giftIntentId: "11111111-1111-4111-8111-111111111111",
        transactionId: "22222222-2222-4222-8222-222222222222",
...
Deno.test("gift test finalization hash changes when transaction changes", async () => {
  ...
  const second = await hashGiftTestFinalizePayload({ ...base, transactionId: "33333333-3333-4333-8333-333333333333" });
