...
async function matches(
  ...
  switch (rule.targetType) {
    case "Global":
      return true;
    case "Product":
      return !!context.productCode && rule.targetKey === context.productCode;
    case "Segment":
      return !!rule.targetKey &&
        (context.segmentKeys ?? []).includes(rule.targetKey);
    case "Beta":
      return context.beta === true && rule.targetKey === "beta";
    case "Account":
      return !!rule.targetKey && rule.targetKey === context.subjectKey;
    case "Percentage":
      return (await bucket(definition.key, context.subjectKey)) <
...
export async function evaluatePlatformControl(
  ...
  const value = definition.failClosed &&
      definition.kind === "FeatureFlag" &&
