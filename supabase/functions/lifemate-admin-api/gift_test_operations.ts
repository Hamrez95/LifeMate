...
export async function parseGiftTestFinalizePayload(request: Request): Promise<GiftTestFinalizePayload> {
  const body = await objectBody(request);
  const claimTokenHash = String(body.claimTokenHash ?? "").trim().toLowerCase();
  if (!/^[0-9a-f]{64,128}$/.test(claimTokenHash)) {
    throw new ApiError(400, "gift_claim_token_hash_invalid", "claimTokenHash is invalid.");
  }
  const claimTtlHours = body.claimTtlHours == null ? 168 : Number(body.claimTtlHours);
  if (!Number.isInteger(claimTtlHours) || claimTtlHours < 1 || claimTtlHours > 720) {
    throw new ApiError(400, "gift_claim_ttl_invalid", "claimTtlHours must be between 1 and 720.");
  }
  return {
...
export async function hashGiftTestFinalizePayload(payload: GiftTestFinalizePayload): Promise<string> {
