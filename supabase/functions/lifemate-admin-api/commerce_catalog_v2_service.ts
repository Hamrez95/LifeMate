    ...
    async get(query: CommerceCatalogV2Query) {
      ...
      const offers = productIds.length === 0 ? [] : await sql`
        ...
        left join lateral (
          ...
          where o.plan_id is not null and p2.plan_id=o.plan_id
      ...
      const bundles = await sql`
        ...
        where (${query.includeHidden}::boolean or b.status='Published') and b.status <> 'Retired'
        order by b.code,o.product_id,o.code
      `;
      return {
        version: "2026-08-26",
        products: rows.map((row) => ({
          id: String(row.id), code: String(row.code), name: String(row.display_name),
          status: String(row.lifecycle_status), updatedAtUtc: iso(row.updated_at_utc),
          offers: offers.filter((offer) => String(offer.product_id) === String(row.id)).map((offer) => ({
            id: String(offer.id), code: String(offer.code), name: String(offer.display_name),
            durationMonths: Number(offer.duration_months), status: String(offer.status),
            giftEligible: Boolean(offer.gift_eligible), version: Number(offer.version),
            price: offer.price_id ? {
              id: String(offer.price_id), countryCode: offer.country_code == null ? null : String(offer.country_code),
              currency: String(offer.currency), storeProvider: String(offer.store_provider),
              amountMinor: String(offer.amount_minor), effectiveFromUtc: iso(offer.effective_from_utc),
            } : null,
          })),
          policies: policies.filter((policy) => String(policy.product_id) === String(row.id)).map((policy) => ({
            key: String(policy.policy_key), value: policy.value_json, valueType: String(policy.value_type), version: Number(policy.version),
          })),
        })),
        bundles: Object.values(bundles.reduce<Record<string,{id:string;code:string;name:string;status:string;giftEligible:boolean;version:number;items:Array<{offerId:string;offerCode:string;productId:string}>}>>((acc,row) => {
          const id=String(row.id);
          acc[id] ??= { id, code:String(row.code), name:String(row.display_name), status:String(row.status), giftEligible:Boolean(row.gift_eligible), version:Number(row.version), items:[] };
          if (row.offer_id) acc[id].items.push({ offerId:String(row.offer_id), offerCode:String(row.offer_code), productId:String(row.product_id) });
          return acc;
        },{})),
      };
    },
