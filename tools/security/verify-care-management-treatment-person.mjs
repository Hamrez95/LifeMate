...
for (
  ...
  if (!index.includes(marker)) {
    throw new Error(
      `Care Management Person treatment wiring missing: ${marker}`,
    );
  }
}
...
if (routeStart < 0 || routeEnd <= routeStart) {
  throw new Error(
    "Care Management Treatment route block could not be isolated.",
  );
}
...
for (
  ...
  if (treatmentRoutes.includes(forbidden)) {
    throw new Error(
      `Active route regressed to legacy Treatment helper: ${forbidden}`,
    );
  }
}
...
for (
  const marker of [
    ...
    "'treatment_plan',${planId}::uuid,null,now()",
  ]
) {
